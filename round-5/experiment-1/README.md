...
Tests on a spatial-reasoning benchmark whether combining redundant logical paths fixes LLM errors (it can't here, for a structural reason) and whether symbolic logic checking reduces confident wrong answers (it does, but only by abstaining).
...
- `art_f-ofxduZjwSM` — dataset
- `art_2Xp7DiYUxoNo` — algebra-tables
- `art_ghVQmxVlmOJJ` — synthetic-control
