#!/usr/bin/env python3
"""Builder for the iter-2 implementation-decision dossier.
Holds the answer text + sources as the single source of truth and emits:
  - research_out.json   (downstream contract: title, summary, answer, sources, follow_up_questions)
  - research_report.md  (human-readable mirror)
  - .terminal_claude_agent_struct_out.json (task struct-out)
"""
import json, os

WS = "/ai-inventor/aii_data/runs/run_IuSkWzF0As-P/3_invention_loop/iter_2/gen_art/gen_art_research_1"

TITLE = "Iter-2 Local-Reader / Prolog-Discharge / CLUTRR Implementation-Decision Dossier"

SUMMARY = (
"Executor-ready, web-verified resolution of the six NEW implementation decisions the iter-2 "
"closure-certified text-to-logic experiments need (beyond the iter-1 dossier art_aQ2Rf8rwqteI). "
"(1) LOCAL-READER PROTOCOL: minimal-span unit = the single sentence containing each event mention "
"(default; +/-1 sentence as a sensitivity arm), grounded in TimeBank-Dense's own annotation window "
"(same-sentence + immediately-following-sentence + DCT; 5 relations BEFORE/AFTER/INCLUDES/IS_INCLUDED/"
"SIMULTANEOUS + VAGUE=underdetermined); 'no shared span' = mentions never co-occur within the window => "
"DEDUCTION-REQUIRED, computable WITHOUT the LLM (T0-safe); disjunctive Pairwise read prompt adapted from "
"Wei et al. arXiv:2407.19568 (Bulk/Iterative/Event-Ranking/Pairwise) + METRE arXiv:2408.07353 multi-label, "
"enumerate base relations + explicit UNIVERSAL option, 'name every relation the text does NOT rule out'. "
"(2) DISCHARGE: SWI-Prolog via pyswip v0.3.3 (class-method Prolog.assertz/query, Python>=3.9, needs apt "
"swi-prolog>=8.4.2, ctypes shared-lib + SWI_HOME_DIR/LIBSWIPL_PATH) PRIMARY, robust subprocess fallback "
"`swipl -s f.pl -g goal -t halt` (exit 0/1/2), clingo 5.8.0 ASP as secondary; QCN encoded as edge(E1,E2,RelSet) "
"+ algebra-seeded compose/3,converse/2; readback |R|==1 emit / |R|>1 ABSTAIN / |R|==0 unsound-flag; "
"CONFIDENT-WRONG = nonabstained-single-relation-mismatch-rate on the deduction-required/absent subset, "
"matched-coverage, paired-bootstrap CI, pre-registered MDE, aligned to Logic-LM/LINC executable-rate practice. "
"(3) CLUTRR: HF CLUTRR/v1 (14 configs, target 0-20 mapping, fields incl proof_state/f_comb/story_edges/edge_types) "
"BUT datasets>=4.0 dropped scripts => load via huggingface_hub snapshot_download of the per-task CSVs (git-LFS) or "
"pin datasets<4.0 + trust_remote_code; kinship composition table = rules_store.yaml primitive compositions + "
"relations_store.yaml surface<->primitive<->gender map (no-relation is a native primitive); absent-relation pairs "
"CONSTRUCTED from rob_train_disc disconnected components / cross-family pairs; atomic-gold = story_edges+edge_types. "
"(4) STRONGER READER: weak anchor = iter-1 google/gemini-3.1-flash-lite ($0.25/$1.50); recommend deepseek/deepseek-v3.2 "
"($0.2288/$0.3432, cross-family, CHEAPER, gate test ~$1.3) as budget-optimal stronger reader, google/gemini-3-flash-preview "
"($0.50/$3.00 thinking) as the clear capability-jump option on a stratified subsample; total two-reader run < $10 with "
"disk cache + prompt caching + hard cost-guard. (5) SEVEN local-regime baselines each re-specified to read ONLY local "
"spans with a matched single-relation abstention signal (raw verbalized-confidence, CoT, self-consistency vote-margin, "
"LINC formalization-agreement, PoT path-agreement [confirmed per-path INDEPENDENT, no cross-path intersection], "
"ILP-commit Eirew M=5, naive single-pass intersection), all thresholded to the SAME coverage object. (6) Four recent "
"temporal/logical-consistency citations pinned (arXiv:2510.15513, 2502.11425, 2412.16100, 2409.14065) + a crisp "
"novelty statement (disjunction-preserving abstain-on-collapse OUTPUT CONTRACT + gold-free closure CERTIFICATE + "
"redundancy-as-coding-rate, NOT the algebra). Includes a consolidated decision table, exact IDs/URLs, and a 12-item "
"gotchas list."
)

ANSWER = r"""# Iter-2 Implementation-Decision Dossier — Local-Reader, Prolog/ASP Discharge, CLUTRR, Stronger Reader, Local-Regime Baselines, Novelty
...
SOURCES = [
 (1, "file://.../iter_1/gen_art/gen_art_research_1/research_out.json", "Iter-1 Implementation/Resource Dossier (dependency art_aQ2Rf8rwqteI)", "Carried-forward verified facts: NarrativeTime/TDDMan/MATRES formats, GQR Allen/point/RCC-8 composition+converse tables and the convex-point widening rule, Mackworth PC-2 closure + naive single-pass contrast, Renz-Nebel generator, baseline list (incl. Eirew ILP-commit repo, Kougia arXiv:2406.11486 >=50-97% multi-relation + ILP-no-F1), OpenRouter cost-guard + disk-cache, realism statistic, T0 zero-spend gate, iter-1 measured $0.58/4191 calls."),
 (2, "https://arxiv.org/abs/2407.19568", "Are LLMs Good Annotators for Discourse-level Event Relation Extraction? (Wei, Gautam, Huang 2024)", "Defines four prompt patterns (Bulk/Iterative/Event-Ranking/Pairwise); Pairwise = query all event-mention pairs; retains first TWO sentences to cover same- and cross-sentence pairs (grounds the local-span window + no-shared-span cut); LLMs underperform, fabricate mentions, miss transitivity and long-distance relations."),
 (3, "https://arxiv.org/abs/2408.07353", "Only One Relation Possible? Modeling Ambiguity in Event Temporal Relation Extraction / METRE (Hu, Huang, Feng 2024)", "Multi-label modeling that infers each relation's possibility independently and treats VAGUE as >1 possible relation -> template for the disjunctive sound local read; no public repo found."),
 (4, "https://arxiv.org/abs/2412.17963", "Path-of-Thoughts: Extracting and Following Paths for Robust Relational Reasoning (Zhang et al., TMLR 2026)", "VERIFIED per-path INDEPENDENCE: 'reasons over each path independently', 'infers all possible answers', 'allowed to output multiple possible relations' (UNION, not cross-path intersection); CLINGO/ASP backend; CLUTRR+StepGame+SPARTUN; up to +21.3%; no public repo (reimplement)."),
 (5, "https://arxiv.org/abs/2502.11425", "Counterfactual-Consistency Prompting for Relative Temporal Understanding (Kim & Hwang, ACL 2025 short)", "Counterfactual questions + collective constraints to fix before/after confusion; prompting-time consistency fix that still commits a single label (novelty contrast)."),
 (6, "https://arxiv.org/abs/2510.15513", "Temporal Referential Consistency: Do LLMs Favor Sequences Over Absolute Time References? (Bajpai & Chakraborty, EMNLP 2025)", "TEMP-ReCon benchmark (EN/FR/RO) + UnTRaP reasoning-path-alignment model; measures sequence-vs-absolute time-reference consistency (novelty contrast: accuracy objective, no algebra/certificate/abstention)."),
 (7, "https://arxiv.org/abs/2412.16100", "Logical Consistency of Large Language Models in Fact-checking (Ghosh et al., ICLR 2025)", "Negation/conjunction/disjunction consistency over KG queries + SFT improvement; propositional-logic consistency under a commit objective (novelty contrast)."),
 (8, "https://arxiv.org/abs/2409.14065", "Temporally Consistent Factuality Probing for LLMs (Bajpai et al., EMNLP 2024)", "TEMP-COFAC paraphrase dataset + CoTSeLF (instruction tuning + consistency RL); paraphrase-level factual consistency (novelty contrast)."),
 (9, "https://huggingface.co/datasets/CLUTRR/v1/blob/main/README.md", "CLUTRR/v1 dataset README (HuggingFace)", "14 configs; fields id/story/query/target/target_text/clean_story/proof_state/f_comb/task_name/story_edges/edge_types/query_edge/genders/task_split; numeric target 0-20 -> relation mapping; hop=task_name clause_length; split sizes (gen_train234 = 12064/3019/1048; gen_train23 = 9074/2020/1146)."),
 (10, "https://huggingface.co/datasets/CLUTRR/v1/tree/main", "CLUTRR/v1 repository file tree (HuggingFace)", "Repo contains README.md + loading script v1.py + per-task CSV files via git-LFS (e.g. task_1.3.csv) -> CSVs can be snapshot-downloaded directly, bypassing the broken loading script."),
 (11, "https://raw.githubusercontent.com/facebookresearch/clutrr/master/clutrr/store/rules_store.yaml", "CLUTRR rules_store.yaml (kinship composition rules)", "Primitive-level composition table (child+child->grand, child+SO->in-law, inv-child+child->sibling, sibling+child->un, grand+sibling->grand, etc.) with sections symmetric/equivalence/inverse-equivalence/compositional/relation_types; FINITE table, not a full algebra."),
 (12, "https://raw.githubusercontent.com/facebookresearch/clutrr/master/clutrr/store/relations_store.yaml", "CLUTRR relations_store.yaml (surface<->primitive<->gender map)", "Maps each surface kinship term to (primitive, gender): son=(child,m), father=(inv-child,m), grandmother=(inv-grand,f), aunt=(inv-un,f), niece=(un,f), son-in-law=(in-law,m), etc.; no-relation=(no-relation, both) is a native primitive -> enables absent-relation gold."),
 (13, "https://pyswip.readthedocs.io/en/stable/get_started.html", "PySwip Get Started documentation", "Requires Python>=3.9 and SWI-Prolog>=8.4.2 installed separately; architecture must match; class-method API Prolog.assertz()/Prolog.query(); pip install -U pyswip; manual SWI_HOME_DIR/LIBSWIPL_PATH if auto-discovery fails."),
 (14, "https://github.com/yuce/pyswip", "yuce/pyswip GitHub repository", "Latest release v0.3.3 (23 Jun 2025); uses SWI-Prolog as a shared library via ctypes; pip install -U pyswip."),
 (15, "https://pypi.org/project/clingo/", "clingo on PyPI", "clingo 5.8.0 (3 Apr 2025); pip install clingo; CFFI bindings; Control (ground/solve), Model, Symbol classes for ASP."),
 (16, "https://potassco.org/clingo/python-api/5.8/clingo/", "clingo 5.8 Python API documentation (Potassco)", "Control grounds/solves; Model gives model access; #show surfaces answer atoms; the ASP alternative discharge engine."),
 (17, "https://openrouter.ai/google/gemini-2.5-flash", "OpenRouter — Gemini 2.5 Flash pricing", "$0.30/M input, $2.50/M output; prompt caching (60-80% cheaper repeated context); 1M context; roughly capability-parity with 3.1-flash-lite (not a valid 'stronger' reader)."),
 (18, "https://openrouter.ai/google/gemini-3-flash-preview", "OpenRouter — Gemini 3 Flash Preview pricing", "$0.50/M input, $3.00/M output; Gemini-3 thinking model (minimal/low/medium/high), structured output, automatic context caching; 1M context; clear capability-jump stronger-reader option."),
 (19, "https://openrouter.ai/google/gemini-3.1-flash-lite", "OpenRouter — Gemini 3.1 Flash Lite pricing", "$0.25/M input, $1.50/M output; 1M context; iter-1 reader = the WEAK anchor for the recall-ceiling test."),
 (20, "https://openrouter.ai/google/gemini-3.5-flash", "OpenRouter — Gemini 3.5 Flash pricing", "$1.50/M input, $9.00/M output; strongest Flash tier; prompt caching; pricey -> subsample only."),
 (21, "https://openrouter.ai/deepseek/deepseek-v3.2", "OpenRouter — DeepSeek V3.2 pricing", "$0.2288/M input, $0.3432/M output; 131K context; prompt caching; cross-family (DeepSeek) frontier MoE -> recommended budget-optimal STRONGER reader (output ~4.4x cheaper than flash-lite)."),
 (22, "https://openrouter.ai/google/gemini-2.5-flash-lite", "OpenRouter — Gemini 2.5 Flash Lite pricing", "$0.10/M input, $0.40/M output; still LIVE (contradicts iter-1 'GONE' note); cheapest option."),
 (23, "https://arxiv.org/abs/2305.12295", "Logic-LM: Empowering LLMs with Symbolic Solvers (Pan, Albalak, Wang, Wang 2023)", "LLM->symbolic formulation->external solver (Prover9 for FOL returns True/False/Unknown); near-linear correlation between executable rate and accuracy; self-refinement from solver error messages -> aligns the confident-wrong metric to prior discharge practice."),
 (24, "https://github.com/benlipkin/linc", "LINC: Neurosymbolic FOL prover approach (Olausson et al., EMNLP 2023; arXiv:2310.15164)", "LLM->FOL->Prover9; returns {True, False, Uncertain} or raises an exception on malformed FOL; majority vote across multiple formalizations -> baseline B4 abstention = formalization agreement; public repo (runner.py)."),
 (25, "https://direct.mit.edu/tacl/article/doi/10.1162/tacl_a_00754/131566/Know-Your-Limits-A-Survey-of-Abstention-in-Large", "Know Your Limits: A Survey of Abstention in Large Language Models (TACL, MIT Press)", "Framework for abstention (query/model/human-values) as hallucination mitigation; basis for thresholded matched-coverage abstention; documents verbalized-confidence over-confidence."),
 (26, "https://arxiv.org/abs/2601.07767", "Are LLM Decisions Faithful to Verbal Confidence? (Wang & Zhou)", "Documents systematic verbalized-confidence over-confidence (clusters 80-100% regardless of accuracy; RLHF amplifies) -> abstention must sweep thresholds, never trust raw numbers."),
 (27, "https://discuss.huggingface.co/t/dataset-scripts-are-no-longer-supported/163891", "HuggingFace: 'Dataset scripts are no longer supported' (datasets>=4.0/4.5)", "datasets>=4.0 removed trust_remote_code and >=4.5 removed dataset-script support -> load_dataset('CLUTRR/v1', cfg) fails; must use CSV snapshot_download or pin datasets<4.0."),
 (28, "https://timeml.github.io/site/publications/timeMLdocs/timeml_1.2.1.html", "TimeML Specification 1.2.1", "EVENT eid=e<int> + class; MAKEINSTANCE auxiliary tag with eiid/eventID/tense/aspect/polarity/modality -> recover per-event mentions for sentence-offset mapping."),
 (29, "https://www.researchgate.net/publication/270878550_An_Annotation_Framework_for_Dense_Event_Ordering", "An Annotation Framework for Dense Event Ordering / TimeBank-Dense (Chambers/Cassidy et al. 2014)", "Gold annotated for all pairs in the SAME sentence + the IMMEDIATELY FOLLOWING sentence + DCT; relations BEFORE/AFTER/INCLUDES/IS_INCLUDED/SIMULTANEOUS/VAGUE; VAGUE = no clear relation (the universal/underdetermined option) -> grounds the local-span window and the no-shared-span deduction trigger."),
 (30, "https://www.swi-prolog.org/pldoc/man?section=cmdline", "SWI-Prolog command-line options manual", "Non-interactive batch: swipl -s file.pl -g 'goal' -t halt; exit code 0 on success, 1 on failure, 2 on exception -> robust subprocess discharge fallback."),
 (31, "https://arxiv.org/abs/1908.06177", "CLUTRR: A Diagnostic Benchmark for Inductive Reasoning from Text (Sinha et al., EMNLP 2019)", "Original CLUTRR; robust-reasoning (§3.5) defines irrelevant, supporting, and DISCONNECTED facts -> disconnected-component entities give guaranteed no-kinship pairs for the absent-relation hallucination demo."),
 (32, "https://github.com/kliang5/CLUTRR_huggingface_dataset", "kliang5/CLUTRR_huggingface_dataset (mirror)", "Script-free per-config CSV mirror of CLUTRR -> alternative direct-CSV access path that avoids the HF loading-script breakage."),
]
...
md = ["# " + TITLE, "", "## Summary", "", SUMMARY, "", "---", "", ANSWER, "", "---", "", "## Sources", ""]
for (i, u, t, s) in SOURCES:
    md.append(f"[{i}] **{t}** — {u}  \n    {s}")
    md.append("")
...
for q in FOLLOW_UPS:
    md.append(f"- {q}")
with open(os.path.join(WS, "research_report.md"), "w") as f:
    f.write("\n".join(md))

# ---- write struct out ----
struct = {
 "title": TITLE,
 "layman_summary": "A verified how-to guide that tells the next engineer exactly which datasets, logic tools, language-model readers, baselines, and citations to use to build and test the hallucination-reducing text-to-logic reasoning pipeline.",
 "summary": SUMMARY,
...
print("WROTE research_out.json, research_report.md, .terminal_claude_agent_struct_out.json")
print("answer chars:", len(ANSWER), "| summary chars:", len(SUMMARY), "| sources:", len(SOURCES))
