...
# ///
"""Canonical entry point for the Natural-Text Absent-Relation Located-In (Administrative
Containment) Corpus.
...
`uv run data.py` does the whole pipeline end-to-end:
  ...
  2. build per-document gold containment graphs (detok.py char offsets + the kinship.py
     closure engine reused VERBATIM under a degenerate single-relation transitive table)
     for the TWO best datasets -- 're-docred' (primary, trustworthy absent gold) and 'docred'
     (secondary, absent gold DOWNGRADED);
  3. standardize to the exp_sel_data_out schema -- ONE ROW PER DOCUMENT, grouped by dataset
     ...
     completeness-correction evidence);
...
$0 LLM spend (pure deterministic data construction; the deterministic cue check passes 100%,
so the optional small-LLM cue judge is skipped).
"""
