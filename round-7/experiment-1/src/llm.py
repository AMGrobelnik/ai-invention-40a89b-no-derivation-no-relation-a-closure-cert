...
class OpenRouterClient:
    ...
    def _key(self, system: str, user: str, temperature: float, tag: str) -> str:
        # tag differentiates repeated samples (self-consistency) WITHOUT changing the prompt;
        # temperature is part of the key so a temp=0 read never collides with a temp=0.7 sample.
        h = hashlib.sha256(
            f"{self._active_model}\x00{temperature}\x00{tag}\x00{system}\x00{user}".encode()
        ).hexdigest()
        return h
    ...
    async def call(self, client: httpx.AsyncClient, sem: asyncio.Semaphore,
                   ...
                   tag: str = "", max_tokens: int | None = None):
        """Return dict {content, cost, cached, model}. Raises BudgetExceeded if hard cap hit.

        temperature/max_tokens override the per-client defaults (per-call); tag separates
        otherwise-identical samples (e.g. self-consistency draws) into distinct cache slots."""
        temperature = self.temperature if temperature is None else temperature
        max_tokens = self.max_tokens if max_tokens is None else max_tokens
        key = self._key(system, user, temperature, tag)
        ...
        async with sem:
            ...
            for attempt, model in enumerate(_with_retries(models)):
                try:
                    data = await self._post(client, model, system, user, temperature, max_tokens)
    ...
    async def run_batch(self, items: list[dict]):
        ...
        async with httpx.AsyncClient(timeout=self.timeout, limits=limits) as client:
            async def one(it):
                try:
                    results[it["id"]] = await self.call(
                        client, sem, it["system"], it["user"],
                        temperature=it.get("temperature"), tag=it.get("tag", ""),
