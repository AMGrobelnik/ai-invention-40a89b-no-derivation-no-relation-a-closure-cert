...
from engine import (Algebra, build_allen_algebra, build_point_algebra, build_rcc8_algebra,
                    naive_single_pass, pc2_full, QCN, RCC8_BASE)
...
# RCC-8: the 8 spatial symbols ARE their own labels (no temporal/spatial rename needed).
RCC8_ANSWER = {s: s for s in RCC8_BASE}        # label -> engine symbol
RCC8_SYM2LAB = {s: s for s in RCC8_BASE}        # engine symbol -> label
RCC8_LAB2SYM = {s: s for s in RCC8_BASE}
RECALL_GATE_RCC8 = 0.85                          # same gate as Allen (used only by --sweep-knob)
...
def build_read_prompt(algebra, knob_key, normalized_sentence):
    ...
    RCC-8 uses a SPATIAL framing; point/allen keep the iter-2 TEMPORAL framing BYTE-FOR-BYTE
    so the copied iter-2 cache reuses every point/allen read (apples-to-apples scaling curve)."""
    if algebra == "rcc8":
        vocab = rcc8_vocab_desc()
        # domain-neutralise the only temporal word in the knob text ("order" -> "relation")
        knob_text = KNOB[knob_key].replace("does not fix the order", "does not fix the relation")
        system = (
            "You are given ONE sentence describing the SPATIAL relationship between two regions, "
            "E1 and E2. Determine how E1 relates to E2 in space. "
            f"The base relations are: {vocab}. "
            f"{knob_text} "
            "Read ONLY from this one sentence; do NOT assume anything about any other regions. "
...
def _answer_format(algebra):
    if algebra == "point":
        ...
        labels = "one of: " + ", ".join(f"'{s}'" for s in RCC8_BASE)
    else:
    ...
    if algebra == "rcc8":
        if mode == "raw":
            ...
            howto = ("First reason step by step, composing the stated relations along the chain(s) "
                     "that connect the two target regions, then give your single answer.")
        else:  # linc
            howto = ("First translate each stated fact into a formal spatial relation, then derive "
                     "the relation between the two target regions by composing those relations along "
                     "the connecting chain(s); then give your single answer.")
        system = (
            "You are given a short report stating SPATIAL relationships among several regions. "
            "Using ONLY these stated facts and transitive spatial reasoning, determine the single "
            "spatial relation between two TARGET regions that are not directly compared in the text. "
        ...
        user = (f"{net['full_doc']}\n\nDetermine the single spatial relation of "
                f"\"{es}\" to \"{et}\" (i.e. how does the first relate to the second in space?).")
        return system, user
    if mode == "raw":
        howto = "Give your best single answer."
    elif mode == "cot":
...
def build_pot_prompt(net, path_sentences, ordered_pairs):
    ...
    if algebra == "rcc8":
        system = (
            "You are given a CHAIN of facts that link a start region to an end region through "
            "intermediate regions. Using ONLY these facts and transitive spatial reasoning along "
            "this single chain, determine the single spatial relation between the two target "
            f"regions. The answer must be {labels}, or 'underdetermined' if this chain alone does "
            "not fix the relation. "
            'Reply with ONLY a JSON object: {"relation": <one label or "underdetermined">, '
            '"confidence": <number 0-1>}.')
        user = (f"Facts (a single chain):\n{facts}\n\n"
                f"Using only this chain, what is the single spatial relation of "
                f"\"{es}\" to \"{et}\"?")
        return system, user
    system = (
...
def parse_answer(content, algebra):
    ...
    if rel_label is None:
        ...
        low = txt.lower()
        if algebra == "point":
            cand_labels = list(POINT_ANSWER)
        elif algebra == "rcc8":
            cand_labels = list(RCC8_BASE)
        else:
            cand_labels = list(ALLEN_LAB2SYM)
        # match case-insensitively (RCC-8 symbols are upper-case); longest first so
        # 'NTPPi'/'TPPi' win over 'NTPP'/'TPP'.
        for lab in sorted(cand_labels, key=len, reverse=True):
            if re.search(r"(?<![a-z])" + re.escape(lab.lower()) + r"(?![a-z])", low):
                rel_label = lab
                break
...
def _label_to_symbol(label, algebra):
    ...
    if algebra == "point":
        ...
        # RCC-8 labels ARE the engine symbols; route through dataio's native parser (handles
        # case + word synonyms, maps 'tppi'->'TPPi', 'ntppi'->'NTPPi').
        s, und, pf = parse_native(json.dumps({"relations": [l]}), "rcc8")
        return next(iter(s)) if (not und and len(s) == 1) else None
    else:
...
def collect_singlelabel_items(nets, knob):
    ...
    for net in nets:
        for (a, b), info in net["edge_sentences"].items():
            ...
            labels = _answer_format(algebra)
            if algebra == "rcc8":
                system = (f"You are given ONE sentence describing the spatial relationship between "
                          f"two regions E1 and E2. Name THE single most likely base spatial relation "
                          f"of E1 to E2. The answer must be {labels}. "
                          'Reply with ONLY {"relation": <one label>}.')
                user = f"{info['normalized']}\nWhat is the single spatial relation of E1 to E2?"
            else:
...
def make_scaling_figure(syn):
    """HEADLINE deliverable (b): Mode-A's matched-coverage advantage over Path-of-Thoughts
    plotted against relation-algebra richness (n base relations): point 3 -> RCC-8 8 ->
    Allen 13. A monotone-rising curve is the central scaling claim."""
    pts = syn.get("scaling_points_vs_pot", []) if syn else []
    pts = [p for p in pts if p.get("gap_vs_pot") is not None]
    if len(pts) < 2:
        return None
    xs = [p["n_base_relations"] for p in pts]
    ys = [p["gap_vs_pot"] for p in pts]
    ys_sc = [p.get("gap_vs_sc") for p in pts]
    labels = {3: "point", 8: "RCC-8", 13: "Allen"}
    fig, ax = plt.subplots(figsize=(7, 4.8))
    ax.plot(xs, ys, "-o", color="#2c7fb8", lw=2, ms=9, label="Mode-A − PoT (selective acc gap)")
    if all(v is not None for v in ys_sc):
        ax.plot(xs, ys_sc, "--s", color="#d95f0e", lw=1.5, ms=7,
                label="Mode-A − Self-Consistency")
    ax.axhline(0, c="k", lw=0.8)
    for x, y in zip(xs, ys):
        ax.annotate(f"{labels.get(x, x)}\nΔ={y:+.3f}", (x, y), textcoords="offset points",
                    xytext=(6, 8), fontsize=9)
    ax.set_xlabel("relation-algebra richness (number of base relations)")
    ax.set_ylabel("selective-accuracy gap @ matched coverage")
    ax.set_xticks([3, 8, 13])
    ax.set_title("Closure advantage scales with algebra richness (REAL-LLM-READ-ON-SYNTHETIC)\n"
                 "RCC-8 numbers are SOUND LOWER BOUNDS (PC-2 sound-but-incomplete)")
    ax.legend(fontsize=8, loc="upper left"); ax.grid(alpha=0.3)
    p = FIGS / "scaling_curve_vs_pot.jpg"; fig.tight_layout()
    fig.savefig(p, dpi=130); plt.close(fig)
    return str(p)


def decomposition_block(per, nets, h1_bite, sound_lower_bound):
    """Deliverable (c): split the system-level Mode-A − PoT gap into
       * inherited_table_vs_llm = selacc(naive single-pass) − selacc(PoT)  (the INHERITED NeSy
         premise: even ONE exact-table composition step beats LLM per-path composition), and
       * novel_iteration        = full PC − naive single-pass resolve-correct on the
         deduction-DEEP strata (hop>=3 / cyclomatic>=1)               (the NOVEL contribution).
    Both reported with bootstrap CIs, on REAL reads and on the clean GOLD-reads reference."""
    DEEP_CELLS = ["hop_L3_P2", "hop_L4_P2", "hop_L5_P2", "cyc_mu1", "cyc_mu2", "cyc_mu3"]
    lb = h1_bite.get("leaderboard", {}) if h1_bite else {}
    sel_modeA = lb.get("modeA", {}).get("selective_accuracy")
    sel_pot = lb.get("pot", {}).get("selective_accuracy")
    sel_naive = lb.get("naive", {}).get("selective_accuracy")
    total_vs_pot = (sel_modeA - sel_pot) if (sel_modeA is not None and sel_pot is not None) else None
    inherited = (sel_naive - sel_pot) if (sel_naive is not None and sel_pot is not None) else None

    def _deep_iteration_gap(keyfull, keynaive):
        deep = [per[n["net_id"]] for n in nets
                if n["net_id"] in per and n["cell"] in DEEP_CELLS]
        if not deep:
            return {"n": 0, "gap_mean": None, "ci95": [None, None]}
        full = np.array([r[keyfull]["correct"] for r in deep], float)
        naive = np.array([r[keynaive]["correct"] for r in deep], float)
        gaps = full - naive
        m, ci = st.mean_ci(gaps, B=BOOT_B, seed=SEED)
        return {"n": len(deep), "gap_mean": float(m), "ci95": ci,
                "full_resolve_correct": float(full.mean()),
                "naive_resolve_correct": float(naive.mean())}

    novel_real = _deep_iteration_gap("modeA", "naive")
    novel_gold = _deep_iteration_gap("gold_full", "gold_naive")
    return {
        "evidence_tag": "REAL-LLM-READ-ON-SYNTHETIC",
        "sound_lower_bound": bool(sound_lower_bound),
        "total_vs_pot_gap": total_vs_pot,
        "inherited_table_vs_llm_gap": inherited,
        "inherited_is": ("premise inherited from neuro-symbolic literature: a single exact "
                         "composition-table step (naive single-pass) already beats LLM per-path "
                         "composition (naive > PoT)."),
        "novel_iteration_gap_real": novel_real,
        "novel_iteration_gap_gold": novel_gold,
        "novel_is": ("the contribution of THIS method: iterated PC closure (full) resolves "
                     "deduction-deep queries (hop>=3 / cyclomatic>=1) that a single naive pass "
                     "leaves underdetermined (full > naive)."),
        "note": ("decomposition selective accuracies are on the bite-bearing pool; "
                 + ("RCC-8 full/naive resolve-correct rates are SOUND LOWER BOUNDS (PC-2 "
                    "sound-but-incomplete)." if sound_lower_bound else
                    "point arm is PC-complete/exact; Allen is a sound lower bound.")),
    }
...
def _sym_label(sym, algebra):
    ...
    if algebra == "rcc8":
        return RCC8_SYM2LAB.get(sym, sym)
...
def main():
    ...
    ap.add_argument("--algebras", default="point,allen,rcc8")
    ap.add_argument("--folds", default="test")
    ap.add_argument("--n-point", type=int, default=90)
    ap.add_argument("--n-allen", type=int, default=90)
    ap.add_argument("--n-rcc8", type=int, default=120)
    ...
    for algebra in algebras:
        ...
        # RCC-8 PC-2 is sound-but-INCOMPLETE -> every closure-derived count (coverage,
        # collapse/Mode-B detection, resolve-to-singleton) is a SOUND LOWER BOUND. The point
        # arm is PC-complete (exact); Allen is also a sound lower bound (NP-hard consistency).
        sound_lower_bound = (algebra in ("rcc8", "allen"))
        decomp = decomposition_block(per, nets, h1_bite, sound_lower_bound)
        if algebra == "point":
            completeness = "PC-COMPLETE (exact): coverage/collapse are EXACT."
            ev_tags = ["REAL-LLM-READ-ON-SYNTHETIC"]
        elif algebra == "rcc8":
            completeness = ("PC-SOUND-BUT-INCOMPLETE: coverage / Mode-B collapse / resolve-to-"
                            "singleton are SOUND LOWER BOUNDS (true minimal network may resolve "
                            "MORE). Every singleton Mode-A commits is still sound (contains gold "
                            "when reads are sound) -> zero-FP-conditional-on-soundness ~1.0.")
            ev_tags = ["REAL-LLM-READ-ON-SYNTHETIC", "SOUND-LOWER-BOUND"]
        else:  # allen
            completeness = ("PC-SOUND-BUT-INCOMPLETE (Allen consistency NP-hard): coverage / "
                            "collapse are SOUND LOWER BOUNDS.")
            ev_tags = ["REAL-LLM-READ-ON-SYNTHETIC", "SOUND-LOWER-BOUND"]
        analysis[algebra] = {
            "evidence_tags": ev_tags,
            "pc_completeness": completeness,
            "n_base_relations": (3 if algebra == "point" else (8 if algebra == "rcc8" else 13)),
    ...
    # HEADLINE figure (b): the algebra-richness scaling curve (point 3 -> rcc8 8 -> allen 13)
    scaling_fig = make_scaling_figure(verdict.get("cross_algebra_synthesis"))
    if scaling_fig:
        verdict["cross_algebra_synthesis"]["scaling_figure"] = scaling_fig
        logger.info(f"Wrote scaling-curve figure: {scaling_fig}")
    ...
    metadata = {
        ...
        "config": {
            ...
            "recall_gate_rcc8": RECALL_GATE_RCC8,
...
def cross_algebra_synthesis(analysis, algebras):
    ...
    # ORDER BY RELATION-ALGEBRA RICHNESS (n base relations), NOT alphabetically:
    #   point (3)  ->  rcc8 (8)  ->  allen (13).
    ordered = [a for a in ("point", "rcc8", "allen") if a in rows]
    gap_pot = [rows[a]["gap_vs_pot"] for a in ordered if rows[a]["gap_vs_pot"] is not None]
    # monotone NON-DECREASING along the richness axis (tol 0.0): each step's gap_vs_pot is
    # >= the previous step's. With the third (RCC-8) point this is the load-bearing curve claim.
    grows = (len(gap_pot) >= 2 and
             all(gap_pot[i + 1] >= gap_pot[i] - 1e-9 for i in range(len(gap_pot) - 1)))
    # also report the simple endpoint inequality (old iter-2 definition) for continuity
    grows_endpoint = (len(gap_pot) >= 2 and gap_pot[-1] > gap_pot[0] + 0.05)
    scaling_points = [{"algebra": a, "n_base_relations": rows[a]["n_base_relations"],
                       "gap_vs_pot": rows[a]["gap_vs_pot"],
                       "gap_vs_sc": rows[a]["gap_vs_sc"],
                       "selacc_modeA": rows[a]["selacc_modeA"],
                       "selacc_pot": rows[a]["selacc_pot"]} for a in ordered]
    return {
        "by_algebra": rows,
        "ordered_by_richness": ordered,
        "scaling_points_vs_pot": scaling_points,
        "finding": ("The closure advantage over neural per-path reasoning scales with relation-"
                    "algebra richness (point 3 -> RCC-8 8 -> Allen 13 base relations): on the "
                    "3-relation convex point algebra a strong neural baseline (Path-of-Thoughts) "
                    "already composes correctly, so Mode-A TIES it (gap~0) while still beating "
                    "self-consistency; on the 8-relation RCC-8 spatial calculus and the 13-relation "
                    "Allen interval algebra (both NP-hard consistency) neural per-path chaining "
                    "degrades and Mode-A's symbolic closure opens a growing gap. RCC-8 closure "
                    "numbers are SOUND LOWER BOUNDS (PC-2 is sound-but-incomplete on RCC-8)."),
        "advantage_grows_with_algebra_richness": bool(grows),
        "advantage_grows_endpoint_gt_0p05": bool(grows_endpoint),
        "monotonicity_note": ("advantage_grows_with_algebra_richness = gap_vs_pot is monotone "
                              "non-decreasing across point<rcc8<allen (tol 0.0); an out-of-order "
                              "RCC-8 point is reported honestly, not suppressed."),
    }
