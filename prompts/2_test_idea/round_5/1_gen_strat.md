...
### [1] SYSTEM-USER prompt · 2026-06-17 22:25:06 UTC
...
id: gen_strat_1_idx1
title: >-
  Run the Decisive Real-Text Cross-Path-Intersection Test (Full-Allen, Multi-Path-Redundant Temporal Queries), Build a Spatial
  Multi-Path Corpus for the Next Venue, Add a Minimal LLM Fuzzy-Unification Demo, and Land the Reviewer's Scope/Reporting
  Fixes
objective: >-
  Settle the single load-bearing open question the reviewer named: does the paper's SIGNATURE NOVEL mechanism -- multi-path
  redundancy as an error-correcting code, where cross-path INTERSECTION of disjunctive LLM reads narrows a query toward gold
  beyond what any single path gives -- actually transfer to natural text, or is it synthetic-channel-only? We attack it head-on
  with the one experiment that can answer it AT POWER: re-mine the existing natural news/temporal corpora under the FULL Allen
  interval algebra (13 relations, rich enough that single-path composition leaves a real disjunction to intersect -- unlike
  the coarse convex-point start-point algebra the prior null used) to isolate genuinely MULTI-PATH-REDUNDANT, non-transitively-trivial
  query pairs, and test whether cross-path INTERSECTION beats BEST-SINGLE-PATH composition with adjusted-CI separation. Simultaneously
  we (a) build a SPATIAL multi-path-redundant gold-QCN corpus (SpartQA-Human/ReSQ natural + SpartQA-Auto/StepGame for power)
  as the building block for next iteration's SECOND real venue -- chosen because rich spatial algebras (cardinal-direction
  / RCC-8) over multi-object layouts host the redundancy-with-bite that dense temporal corpora (transitively closed) and sparse
  ones (few paths) struggle to provide; (b) deliver the MINIMAL LLM-as-probabilistic-composition-reasoner ('fuzzy unification')
  demonstration the project goal demands and the reviewer invited -- an LLM resolving composition-table GAPS, honestly scoped,
  with the resolved step flagged in the trace; (c) re-analyze existing outputs to land every reporting-rigor and honest-scope
  fix (R1 CI-bracketing, R2 CLUTRR naive natural-coverage + coverage-axis routing, the 42.5%-confident-wrong-among-answered
  figure, the deduction-sub-module + 0.53-atomic-recall ceiling framing, and the TRANSFERABLE-vs-SYNTHETIC-ONLY evidence boundary);
  and (d) pin GDLLM and BeDiscovER with a sharpened one-sentence novelty. If the decisive test is POSITIVE, the novel coding
  mechanism is validated on real text and the headline is genuinely earned; if NEGATIVE, the headline is honestly restructured
  to 'inherited exact-table multi-hop composition + a gold-free, training-free, per-edge abstain-on-collapse certificate (real
  text) PLUS a cross-path-intersection coding mechanism (synthetic-channel)', with the spatial corpus already staged to retry
  the mechanism in iteration 5.
rationale: >-
  The reviewer's #1 MAJOR is unambiguous and is now also the hypothesis's organizing principle: the cross-path-intersection
  coding mechanism is powered ONLY on the synthetic channel. CLUTRR cannot test it (kinship has no involutive converse, so
  the sound engine is a forward UNION fixpoint over single chains -- the Lena->Irvin->James->Lynn case is sequential transitive
  composition, NOT cross-path intersection; art_0a7i481ZRwS1), and the prior temporal test (art_OETjJkketEVS) found the iteration
  signature non-significant (+0.027, p=0.079). Crucially, that temporal test ran in the COARSE convex-point start-point algebra
  (3 relations), where single-path reads are near-singletons and dense gold is transitively closed, so full closure trivially
  equals naive single-pass -- there is nothing for intersection to narrow. The decisive lever, available NOW with zero new
  data, is the FULL Allen algebra: the frozen temporal gold graphs (art_PNrS9T8JeATf) already carry per-edge `canonical_relation_set`
  (Allen-13) alongside `startpoint_relation_set` (point-3), so we can read 13-relation disjunctive sets span-locally, and
  then single-path composition leaves a genuine disjunction whose intersection with a SECOND disjoint path can narrow toward
  gold. This is the exact mechanism, on genuinely natural news text, with a NEW and sharper baseline (best-single-path) that
  isolates 'intersection beyond the single best path' -- which no prior experiment ran. Power is available: NarrativeTime
  alone has 1.58M event-event triangles and (point-algebra) N*=25,450, so the multi-path-redundant Allen subset should clear
  a 100-150-query bar; if it does not, that itself is an honest, reportable scope result. We hedge the mechanism's real-text
  fate across TWO venues: the temporal test runs now, and the spatial corpus is built now so iteration 5 can retry on the
  venue most structurally hospitable to multi-path bite (rich directional/region algebras + multi-object scenes give many
  edge-disjoint constraining paths between a query pair, exactly what temporal corpora lack). The fuzzy-unification demo converts
  a MAJOR scope criticism into a constructive, novel deliverable that touches the project goal's 'LLM as probabilistic reasoning
  engine for fuzzy unifications / logical gaps' requirement -- currently entirely absent -- without overclaiming OpenCyc grounding.
  The evaluation and citation artifacts retire the remaining MINORs at zero LLM spend and provide GEN_PAPER_TEXT the corrected
  tables and honest framing. Every experiment reuses the validated engine (Allen-169 cells + convex-point completeness + RCC-8-64
  cells unit-tested in art_K7riobQ_Rmwz/art_QToTkRe6Umb8), the cached cost-guarded OpenRouter client, and the matched-coverage/doc-clustered-bootstrap
  harness from prior workspaces, so this iteration is execution-and-analysis under a hard <$10 cap, not new infrastructure.
  All five artifacts run concurrently and combine into one pool that either earns the novel-mechanism headline on real text
  or restructures it honestly while staging the second venue.
artifact_directions:
- id: experiment_iter4_dir1
  type: experiment
  objective: >-
    THE DECISIVE EXPERIMENT (retires reviewer MAJOR #1). On genuinely natural text (news-derived temporal corpora) and at
    statistical power, test whether cross-path INTERSECTION of disjunctive LLM reads narrows real-text deduction queries strictly
    BEYOND best-single-path composition -- the paper's claimed novel error-correcting-code mechanism. Use the FULL Allen interval
    algebra (13 relations) rather than the coarse convex-point start-point restriction, because the latter is precisely why
    the prior test (art_OETjJkketEVS) found full==naive on dense, transitively-closed gold (single-pass already resolved).
    Deliver a CONFIRM (intersection > best-single-path, adjusted-CI separated from 0, at n>=100-150 multi-path-redundant queries)
    or a defensible SCOPE-BOUNDARY (no separation => the coding mechanism is honestly synthetic-only and the real-text contribution
    is the inherited composition + certificate).
  approach: >-
    Operate on the frozen real temporal gold graphs [ARTIFACT:art_PNrS9T8JeATf], whose edges already carry full `canonical_relation_set`
    (Allen-13), `coarse_superset_set`, and `startpoint_relation_set`; use NarrativeTime (dense news, richest Allen disjunctions
    -> primary host) and TDDMan (manual long-distance, non-circular corroboration). Reuse the validated bitmask Allen engine
    + Mackworth PC-2 closure from /ai-inventor/aii_data/runs/run_IuSkWzF0As-P/3_invention_loop/iter_1/gen_art/gen_art_experiment_1
    (engine.py, tests.py) and the span-local read / matched-coverage / doc-clustered-paired-bootstrap harness + cached cost-guarded
    OpenRouter client from .../iter_3/gen_art/gen_art_experiment_2 (data_adapter.py, llm.py), per the engine/algebra specs
    [ARTIFACT:art_aQ2Rf8rwqteI]. STEP 1 -- A-PRIORI MULTI-PATH GATE (zero LLM): from the gold graphs enumerate held-out query
    edges (s,t) with >=2 EDGE-DISJOINT constraining paths such that (i) the best single path's gold-Allen composition is itself
    NON-SINGLETON (so single-path leaves a disjunction = room to narrow; this is the 'non-transitively-trivial' filter that
    excludes queries a single path already resolves) AND (ii) the INTERSECTION of the disjoint paths' compositions strictly
    tightens toward (and contains) the gold relation. Report N_multipath, the >=2-disjoint-path prevalence, the bite distribution,
    and a paired-bootstrap power/MDE calc; pre-register the >=100-150-query power target and a go/no-go. This is a DIFFERENT,
    sharper gate than the prior point-algebra N* (it is intersection-of-disjoint-paths-specific, not transitive-recovery-specific).
    If NarrativeTime+TDDMan together cannot reach power, report that honestly and route the mechanism's real-text validation
    to the spatial venue next iteration. STEP 2 -- REAL LLM DISJUNCTIVE ALLEN READS: prompt google/gemini-3.1-flash-lite (temp
    0; deepseek-v3.2 cross-family sensitivity on a bounded subsample) to emit, per span, a high-recall SOUND Allen disjunction
    (enumerate the 13 base relations + an explicit universal/underdetermined option, 'name every relation the text does not
    rule out'); SHA-256 disk cache + hard cost guard; track cumulative spend, cap well under $10. STEP 3 -- COMPARE at MATCHED
    single-relation coverage with doc-clustered paired-bootstrap, Holm-adjusted: (a) CROSS-PATH INTERSECTION = full iterated
    closure intersecting all disjoint paths (Mode A, our mechanism); (b) BEST-SINGLE-PATH composition = the most-constraining
    single path's composed set (THE critical new baseline isolating 'intersection beyond the single best path'); (c) NAIVE
    single-pass query-node intersection; (d) Path-of-Thoughts (per-path independent, modal vote, path-agreement abstain);
    (e) raw local LLM (forced single). HEADLINE CONTRAST = (a) vs (b): singleton-resolution-to-correct gap with an adjusted
    CI that BRACKETS the observed gap (percentile of the matched-gap distribution -- explicitly fixing reviewer MINOR R1,
    whose prior CI [0.045,0.315] excluded the +0.0265 point estimate). HONESTY: full Allen PC is sound-but-INCOMPLETE so coverage/collapse
    numbers are SOUND LOWER BOUNDS, but intersection-of-sound-sets is always sound so the narrowing is valid; tag every number
    REAL-LLM-READ; report per-edge recall vs the 0.85 Allen gate, within-doc error correlation rho, and the confident-wrong-among-answered
    rate. Emit method_out.json (aii-json validated, exp_gen_sol_out) with per-query predict_intersection/best_single_path/naive/pot/raw
    + gold + path-structure metadata (n_disjoint_paths, per-path composed set, bite, singleton_resolved), the a-priori gate
    table, the intersection-vs-best-single-path leaderboard with bracketing adjusted CIs, and an explicit CONFIRM / SCOPE-BOUNDARY
    verdict.
  depends_on:
  - id: art_PNrS9T8JeATf
    label: dataset
  ...
  - id: art_aQ2Rf8rwqteI
    ...
    relation_type:
    relation_rationale:
- id: dataset_iter4_dir2
  type: dataset
  objective: >-
    Build a SPATIAL multi-path-redundant gold-QCN corpus -- the building block for iteration 5's SECOND real-venue decisive
    test of cross-path intersection. Spatial layouts are the venue most structurally able to host the novel mechanism: rich
    spatial algebras (cardinal-direction calculus, RCC-8) over scenes with many objects naturally yield query pairs reachable
    via >=2 EDGE-DISJOINT constraining paths whose single-path compositions leave disjunctions that intersection narrows --
    the redundancy-with-bite that dense temporal corpora lack (transitively closed) and sparse ones barely have (~12 edges).
    Report, a-priori and without any LLM, the multi-path-redundancy prevalence so next iteration can decide go/no-go before
    spending budget.
  approach: >-
    Source spatial-reasoning corpora via aii-hf-datasets and official repos, prioritizing HUMAN-AUTHORED / natural text for
    the 'genuinely natural' claim and adding templated sets for power (each flagged honestly): SpartQA (Mirzaee et al. 2021
    -- SpartQA-Human natural + SpartQA-Auto for power), ReSQ (Mirzaee & Kordjamshidi 2022 -- real-world spatial QA), and StepGame
    (Shi et al. 2022 -- cardinal-direction multi-hop, templated, with noise variants that inject redundant paths); evaluate
    SpaRTUN/SpaRP as fallbacks. Verify existence, license, format, relation vocabulary, and document length (<=300MB; report
    any that approach the 3000-char target the temporal/CLUTRR venues miss). Standardize the best 2 to the aii exp_sel_data_out
    schema, ONE ROW per scene/story, mirroring the temporal/CLUTRR gold-graph conventions and the schema in [ARTIFACT:art_aQ2Rf8rwqteI]:
    nodes = objects/entities with surface + mention char-offset spans; edges = native spatial relation -> canonical algebra
    mapping, splitting MEREOTOPOLOGICAL relations (contains/inside/touching/disjoint/overlap -> RCC-8, which the engine already
    validates) from DIRECTIONAL relations (left/right/above/below + diagonals -> cardinal-direction calculus, vocabulary recorded
    for next-iter engine extension), each as a high-recall SOUND disjunctive superset with an explicit universal option; a
    held-out multi-hop query edge; document/scene folds. CRUCIAL DELIVERABLE (DESCRIPTIVE only -- no closure, no LLM, no derived
    P/R): per query edge, count the number of EDGE-DISJOINT constraining paths, hop length, and cyclomatic number, and report
    the corpus-level fraction of held-out queries with >=2 disjoint paths -- the spatial analog of the temporal a-priori gate,
    so we know up front whether genuine multi-path redundancy exists to test. Honest caveats in metadata: which corpora are
    templated vs natural, document-length distribution vs the 3000-char target, and relation-vocabulary coverage of each algebra.
    Emit data.py + full/mini/preview data_out.json (aii-json validated) + a dataset card with the multi-path-redundancy prevalence
    table and provenance/licenses.
  depends_on:
  - id: art_aQ2Rf8rwqteI
    label: schema-specs
    relation_type:
...
- id: experiment_iter4_dir3
  type: experiment
  objective: >-
    Deliver the MINIMAL LLM-as-probabilistic-composition-reasoner ('fuzzy unification') demonstration the project goal calls
    for and reviewer MAJOR #3 explicitly invited -- an LLM resolving composition steps where the EXACT symbolic table has
    a GAP -- substantiating the neuro-symbolic framing the motivation invokes WITHOUT overclaiming OpenCyc grounding or general
    fuzzy unification. Quantify the precision/recall/hallucination tradeoff of letting the LLM fill gaps vs abstaining, and
    flag the LLM-resolved step distinctly in the trace.
  approach: >-
    Two clearly-scoped, honestly-labeled (EXPLORATORY) settings. (1) CLEAN COMPOSITION-ACCURACY BENCHMARK on the synthetic
    Allen backbone [ARTIFACT:art_ghVQmxVlmOJJ], where the TRUE 13x13 (and RCC-8 64-cell) composition table is known: ABLATE
    K% of defined composition cells to simulate table gaps, and when closure hits an ablated cell query an LLM (google/gemini-3.1-flash-lite,
    temp 0) to probabilistically compose the two input relations; score LLM composition accuracy EXACTLY against the withheld
    true cell, stratified by algebra richness (point/RCC-8/Allen) -- a clean 'LLM as composition reasoner' accuracy curve.
    (2) END-TO-END GAP-FILLING DEMO on CLUTRR kinship [ARTIFACT:art_HS7-lxhZnU9m] following the prompt/loader guidance in
    [ARTIFACT:art_Dm5vYXmD1R8h]: reuse the forward-union closure engine (kinship.py from .../iter_3/gen_art/gen_art_experiment_1)
    and identify composition steps the finite kinship table leaves UNDEFINED or genuinely AMBIGUOUS (the documented inv-child
    vs inv-in-law cells that drive the engine's 1.5% abstentions); when the symbolic derivation cannot resolve such a step,
    query the LLM to fill it probabilistically, optionally conditioning on the local story context for disambiguation. MEASURE:
    (i) end-to-end queries RECOVERED that pure-symbolic abstains on, and the ADDED confident-wrong (hallucination) cost, reported
    as a risk-coverage tradeoff vs the abstain-always symbolic baseline (so gap-filling cannot win by simply answering more);
    (ii) the LLM-resolved-step accuracy where gold is known; (iii) a TRACE-GRAPH that flags each LLM-resolved (fuzzy) step
    distinctly from exact-table steps, preserving auditability. HONESTY: state plainly this is composition-cell gap-filling,
    NOT OpenCyc/upper-ontology grounding nor general fuzzy unification; tag REAL-LLM-READ; SHA-256 cache + hard cost guard,
    cap well under $10. Emit method_out.json (aii-json validated, exp_gen_sol_out) with per-instance LLM-resolved-cell predictions
    + gold, the composition-accuracy-vs-richness table, the CLUTRR gap-fill risk-coverage curve, a flagged-fuzzy-step worked
    trace, and a verdict on whether minimal LLM fuzzy-unification adds net faithful coverage.
  depends_on:
  - id: art_HS7-lxhZnU9m
    label: dataset
    relation_type:
...
- id: evaluation_iter4_dir4
  type: evaluation
  objective: >-
    Retire the reviewer's reporting-rigor MINORs and re-frame the evidence boundary honestly, at ZERO LLM spend, by re-analyzing
    existing outputs -- giving GEN_PAPER_TEXT corrected tables and the honest TRANSFERABLE-vs-SYNTHETIC-ONLY framing the new
    hypothesis adopts as its organizing principle.
  approach: >-
    Re-analyze the iter-3 method_out files directly from the two source EXPERIMENTS -- powered temporal [ARTIFACT:art_OETjJkketEVS]
    and CLUTRR end-to-end [ARTIFACT:art_0a7i481ZRwS1] -- reproducing and extending the prior decomposition (art_D0cHQUJ8kY75,
    which reported +0.673 inherited / +0.0025 novel-on-selacc; recompute from the experiment predict_* columns rather than
    depending on that evaluation); numpy+scipy only, seed-fixed paired bootstrap B=10000. (R1, MINOR rigor a) Recompute the
    temporal Mode-A-vs-PoT gap CI as a percentile of the MATCHED-GAP distribution that BRACKETS the observed +0.0265 (the
    reported [0.045,0.315] excluded the point estimate due to re-matching coverage inside the bootstrap); document the fix.
    (R2, MINOR rigor b) From the CLUTRR output add NAIVE single-pass's NATURAL-coverage selective accuracy (~0.216) beside
    the force-extended (matched 0.686 -> 0.229) value, FLAG the force-extension explicitly for the table caption, and ROUTE
    the iteration claim through the COVERAGE axis (full-minus-naive coverage gap 0.0@hop-2 -> 0.586@hop-3 -> up to 0.875@hop-9,
    which the data genuinely support) rather than the forced selective-accuracy contrast. (MINOR evidence) Surface the temporal
    confident-wrong-AMONG-ANSWERED figure (42.5%, 48/113) prominently as a metric to sit beside every temporal claim, temper
    'faithfulness-by-abstention', and tie it to the UNDETECTABLE silent-wrong-narrowing failure mode (gold omitted from a
    contributing set -> confident wrong singleton, no collapse). HONEST EVIDENCE BOUNDARY (MAJOR #1/#2 framing): produce a
    single contribution-split table distinguishing TRANSFERABLE-AT-POWER (inherited exact-table multi-hop composition + the
    gold-free, training-free, per-edge abstain-on-collapse certificate; real/semi-real) from SYNTHETIC-CHANNEL-ONLY (the cross-path-intersection
    coding-rate mechanism + inverted-U), re-tag CLUTRR as a 'templated kinship benchmark' (NOT 'non-synthetic'), state the
    decisive end-to-end win is on a templated benchmark while natural-text temporal is marginal, and foreground the deduction-sub-module
    scope + the ~0.53-atomic-recall -> ~19%-Mode-A-coverage ceiling. Re-affirm the H1-H4 Holm/gatekeeping multiplicity policy
    and note that the iteration this-iter decisive experiment introduces a sharper real-text mechanism test (intersection
    vs best-single-path) whose verdict will update the boundary. Emit eval_out.json (aii-json validated, exp_eval_sol_out)
    + eval_digest.md with the corrected CIs, the CLUTRR natural-vs-forced-coverage table, the 42.5% reporting block, and the
    contribution-split + scope-framing guidance for the paper.
  depends_on:
  - id: art_OETjJkketEVS
    label: temporal-data
    relation_type:
...
- id: research_iter4_dir5
  ...
  approach: >-
    Build on the iter-3 citation bundle [ARTIFACT:art_fFOG-OJakRw-] using aii-web-tools (search -> fetch -> fetch_grep) +
    aii-semscholar-bib. (1) CITATIONS: confirm GDLLM (arXiv:2508.20828) and BeDiscovER (arXiv:2511.13095) resolve to real
    papers (find correct replacements if not), fetch verified BibTeX, and extract each one's objective + output contract (single-label
    COMMIT? abductive REPAIR? trained/learned abstention?). Deliver a crisp differentiation: our contribution PRESERVES a
    relation-algebra disjunction and ABSTAINS on closure-collapse (gold-free, training-free, per-edge certificate) versus
    their commit/repair/trained objectives. Sharpen the paper's one-sentence novelty to exactly what is uniquely demonstrated
    end-to-end on a real benchmark (the training-free, per-edge, gold-free abstain-on-collapse certificate over LLM-extracted
    relational facts), SEPARATED CLEANLY from the cross-path-intersection coding-rate thesis (whose real-text status this
    iteration's decisive experiment determines). (2) SPATIAL DOSSIER (for the sibling spatial DATASET this iter and the spatial
    EXPERIMENT next iter): verify SpartQA (Human + Auto), ReSQ, StepGame, and SpaRTUN/SpaRP -- availability (HF/repo), license,
    relation vocabulary, document length, and whether scenes contain MULTI-PATH redundancy (multiple objects -> multiple edge-disjoint
    constraining paths between a query pair). Identify and source the EXACT composition tables for the relevant spatial algebras:
    the cardinal-direction calculus (Frank 1996 / Ligozat 1998 -- 9 single-tile direction relations + its composition table)
    for directional relations, and confirm the already-validated RCC-8 table for mereotopological relations, flagging any
    cells needing self-test in the next-iter engine extension. Deliver research_out.json with verified BibTeX, per-paper objective
    summaries, the differentiation paragraph + sharpened novelty sentence, ID corrections if any, the spatial-corpus comparison
    table, and the spatial-algebra composition-table sources.
  depends_on:
  - id: art_fFOG-OJakRw-
    label: extends
    relation_type:
    relation_rationale:
expected_outcome: >-
  After this iteration the paper either earns or honestly bounds its signature novelty, and every remaining reviewer critique
  is retired. (1) THE DECISIVE TEST: a powered, full-Allen, multi-path-redundant temporal experiment that for the first time
  compares cross-path INTERSECTION against BEST-SINGLE-PATH composition on genuinely natural text -- yielding either CONFIRM
  (the novel error-correcting-code mechanism transfers to real text, adjusted-CI-separated at n>=100-150, with R1-correct
  bracketing CIs) or a defensible SCOPE-BOUNDARY (mechanism is synthetic-channel-only, real-text contribution = inherited
  composition + gold-free certificate), the exact decision the reviewer demanded. (2) A spatial multi-path-redundant gold-QCN
  corpus (natural SpartQA-Human/ReSQ + templated SpartQA-Auto/StepGame) with an a-priori multi-path-redundancy prevalence
  table, staging iteration 5's SECOND real venue where rich spatial algebras most favor the mechanism. (3) A minimal, honestly-scoped
  LLM fuzzy-unification demo (LLM fills composition-table gaps; clean accuracy-vs-richness curve on synthetic + end-to-end
  risk-coverage on CLUTRR + flagged-fuzzy-step trace) that touches the project goal's 'LLM as probabilistic reasoner for logical
  gaps' requirement without overclaiming OpenCyc. (4) A zero-spend evaluation landing R1 (bracketing CI), R2 (CLUTRR naive
  natural-coverage + coverage-axis routing), the prominent 42.5%-confident-wrong-among-answered reporting, the deduction-sub-module
  + 0.53-recall-ceiling framing, the CLUTRR-as-templated relabeling, and the TRANSFERABLE-vs-SYNTHETIC-ONLY contribution split.
  (5) Verified GDLLM + BeDiscovER BibTeX, a sharpened one-sentence novelty, and a spatial-corpus/algebra dossier for the next
  venue. Net: the mechanism's real-text status is resolved (or staged for a second shot), the headline is correctly attributed,
  the project-goal scope gap is partly addressed by a real demo, and all reporting/scope/citation MINORs are fixed -- all
  under a hard <$10 LLM budget reusing frozen engines, clients, and harnesses.
summary: >-
  Run the one experiment the reviewer demanded: re-mine the natural temporal corpora under the FULL Allen algebra (rich enough
  to leave single-path disjunctions, unlike the coarse point algebra that produced the prior null) to isolate genuinely multi-path-redundant
  non-trivial queries and test at power whether cross-path INTERSECTION beats BEST-SINGLE-PATH -- CONFIRM the novel coding
  mechanism on real text or honestly scope it synthetic-only. In parallel: build a spatial multi-path-redundant corpus (SpartQA/ReSQ/StepGame)
  as next iteration's second real venue; add a minimal, honestly-scoped LLM fuzzy-unification (composition-gap-filling) demo
  touching the project goal; a zero-spend evaluation landing R1 CI-bracketing, R2 CLUTRR naive natural-coverage + coverage-axis
  routing, the 42.5%-confident-wrong reporting, and the deduction-sub-module / extraction-ceiling / templated-CLUTRR / transferable-vs-synthetic
  framing; and a research artifact pinning GDLLM + BeDiscovER withl-llm-read})
\label{sec:temporal}
...
</prompt> a sharpened novelty plus a spatial corpus/algebra dossier.
  Five focused artifacts -- two experiments, one dataset, one evaluation, one research -- that resolve the signature-mechanism
  question and retire every outstanding critique under <$10.
</previous_strategies>
...
## Natural-text temporal: marginal advantage, weakly protective certificate, corrected statistics (\textsc{rea
````

### [2] SYSTEM-USER prompt · 2026-06-17 22:34:06 UTC
...
DEPENDENCY ERRORS (depends_on can ONLY reference IDs from <existing_artifacts>):
  - Strategy 1: Artifact 'experiment_iter5_dir1' (experiment): dependency 'art_QToTkRe6Umb8' has type 'experiment' which is not allowed (allowed: {'research', 'dataset'})
  - Strategy 1: Artifact 'experiment_iter5_dir1' (experiment): dependency 'art_0AIWMhwc1pJM' has type 'experiment' which is not allowed (allowed: {'research', 'dataset'})
  - Strategy 1: Artifact 'experiment_iter5_dir2' (experiment): dependency 'art_OvidVcsfr5HM' has type 'experiment' which is not allowed (allowed: {'research', 'dataset'})
  - Strategy 1: Artifact 'experiment_iter5_dir3' (experiment): dependency 'art_fil2iJ6xSrYx' has type 'experiment' which is not allowed (allowed: {'research', 'dataset'})
  - Strategy 1: Artifact 'experiment_iter5_dir3' (experiment): dependency 'art_0a7i481ZRwS1' has type 'experiment' which is not allowed (allowed: {'research', 'dataset'})
  - Strategy 1: Artifact 'evaluation_iter5_dir4' (evaluation): dependency 'art_Vc1UBGIVSi0T' has type 'evaluation' which is not allowed (allowed: {'dataset', 'experiment'})

INSUFFICIENT VALID ARTIFACTS:
  Required: at least 1 valid artifacts
  Found: 0 valid out of 4 total
