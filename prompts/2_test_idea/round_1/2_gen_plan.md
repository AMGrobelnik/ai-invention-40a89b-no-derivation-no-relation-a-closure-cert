...
### [1] SYSTEM-USER prompt · 2026-06-17 13:25:09 UTC
...
approach: >-
  ...
  edges, multi-path triangles, and >=3-edge/cyclic structures (descriptive only; T0 computes the gated N*). This is the frozen
  reusable foundation; do NOT compute derived gated statistics here.
depends_on: []
</artifact_direction>
...
### [2] HUMAN-USER prompt · 2026-06-17 13:25:09 UTC
...
approach: >-
  ...
  SIMULTANEOUSLY (i) satisfy the structural deduction-required proxy (two events share no local/adjacent-sentence span), (ii)
  are constrained by >=2 paths through distinct intermediates, (iii) retain BITE after non-convex->VAGUE widening, (iv) have
  cross-path intersection equal to the gold singleton. Also compute: the >=3-edge/cyclic structural prevalence (the real-text
  iteration envelope for H3); the candidate count destroyed by widening (bite loss); a paired-bootstrap POWER calc per corpus;
  and — for NarrativeTime — the LOCALLY-JUSTIFIABLE vs PURELY-TIMELINE-IMPLIED split (non-circularity audit, structural proxy
  only). Score each corpus's deduction-required-multi-path-with-bite fraction against the pre-registered applicability NUMBER
  (>=10% general mechanism / 5-10% useful module / <5% niche safety-net). Emit method_out.json (aii-json validated) with the
  per-corpus envelope table, the hosting decision (which corpus is the real-text co-primary, if any clears the number), and
  the expected MATRES N*~0 vs NarrativeTime/TDDMan N*>>0 reported as gate validation. State the go/no-go verdict explicitly.
depends_on: []
</artifact_direction>
...
### [3] SKILL-INPUT — aii-web-tools · 2026-06-17 13:26:49 UTC
...
approach: >-
  ...
  experiments) and (b) an NL REALIZATION — template-verbalize each edge's relation into a short sentence pair so an LLM reader
  can be run on synthetic text in later iterations. Pre-register (record in metadata, do NOT validate yet) the realism-matching
  thresholds (TV distance of per-edge error-type distributions, cross-edge rho match, redundancy/topology histogram match)
  to be validated next iteration against the real frontier-pilot error distributions. Emit schema-valid rows {input: NL realization
  (abstract graph in metadata), output: gold graph, metadata_fold, algebra, redundancy/hop/cycle/node-count cell labels}.
  Validate with aii-json, split full/mini/preview, keep <=300MB.
depends_on: []
</artifact_direction>
...
### [3] SKILL-INPUT — aii-web-tools · 2026-06-17 13:27:45 UTC
...
type: experiment
objective: >-
  De-risk and establish H3 (iterated closure beats naive intersection, gap grows with hop/cyclomatic) and H4 (redundancy inverted-U
  with recall-dependent peak) on clean synthetic ground truth — independent of corpus and LLM-elicitation risk — using a parameterized
  simulated reader so per-edge recall and cross-edge correlation rho are exact knobs.
approach: >-
  ...
  the silent-narrowing COST curve vs redundancy at each fixed recall; MEASURE empirical joint soundness J(E) = realized fraction
  of E-edge constraining subnetworks in which ALL edges are sound (do NOT assume the independence product r^E); the cost term
  is 1 - J(E); locate the inverted-U peak; verify the peak moves OUTWARD with higher recall and shifts under a recall-floor
  gate; confirm positive rho makes J(E) decay slower than r^E so the peak sits further out than the independence model predicts.
  Pre-register estimation precision (>=4 recall levels, >=500 networks/cell, >=1-bin minimum-detectable peak shift, peak-bin
  CI above both neighbors). Also run the zero-FP audit (C3): realized fraction-still-contains-gold vs EMPIRICAL J(E), report
  slope ~1 and apply the attribution rule (offset that disappears under J(E) = independence-approximation failure, not a soundness
  failure). Emit method_out.json (aii-json validated) with gap-vs-hop curves, benefit/cost decomposition, located peaks, audit
  slopes, and a clear PASS/FAIL on H3 and H4 at the pre-registered precision. Zero LLM spend.
depends_on: []
</artifact_direction>
...
approach: >-
  ...
  the NarrativeTime start-point restriction; which non-convex relations widen to VAGUE), and RCC-8 — find authoritative machine-readable
  tables (GQR/SparQ distributions; Allen 1983). (3) Path-consistency spec: Mackworth PC-2 iterated to a fixpoint, converse/identity
  seeding from the algebra, the NAIVE single-pass-intersection variant (no fixpoint, no converse seeding), and tractability
  facts (PC complete for convex point algebra + ORD-Horn; NP-complete for full Allen IA / RCC-8). (4) Renz-Nebel random consistent
  QCN generator A(n,d,l) parameters for INDEPENDENTLY controlled redundancy/density/hop-count/cyclomatic number. (5) Baseline
  specs with a matched abstention signal each: Path-of-Thoughts (arXiv:2412.17963 — confirm per-path INDEPENDENT reasoning
  + path-agreement abstain), self-consistency vote margin, LINC multi-formalization vote (arXiv:2310.15164), DSR-LM induced
  rules (arXiv:2305.03742), a TempRel COMMIT baseline via ILP (arXiv:2502.11114; Knez & Sun arXiv:2406.11486), and the METRE
  multi-label head (arXiv:2408.07353). (6) The cheapest capable OpenRouter model for short-doc disjunctive relation reading
  + a prompt/response caching strategy keeping total spend well under $10. (7) Operationalization of the EXTENDED realism-matching
  statistic (total-variation distance of per-edge error-type distributions + cross-edge rho match + redundancy/topology histogram
  match). Deliver a structured dossier with a decision table and any verified gotchas.
depends_on: []
</artifact_direction>
...
### [1] SYSTEM-USER prompt · 2026-06-17 13:29:54 UTC
...
objective: >-
  ...
  emit SOUND but sub-universal disjunctive relation sets at usable per-edge recall with non-trivial singleton-resolution yield
  — BEFORE committing to the main run. This is the LLM-viability go/no-go.
approach: >-
  ...
  frontier (recall vs singleton-resolution yield vs collapse rate) as a primary deliverable and apply the pre-registered go/no-go:
  proceed to the main run only if a region clears a recall gate AND yields non-trivial singleton-resolution-to-correct. Emit
  method_out.json (aii-json validated) with the frontier table, the SELECTED operating point, measured rho, the empirical
...
### [2] HUMAN-USER prompt · 2026-06-17 13:29:54 UTC
