...
### [1] SYSTEM-USER prompt · 2026-06-17 15:16:14 UTC
...
id: gen_strat_1_idx1
title: >-
  Foundations & Decisive Gates: Corpora, Synthetic Backbone, the Zero-Spend Envelope Gate, and the Recall-Bite Frontier
objective: >-
  Lay the rock-solid, reusable foundation for the closure-certified-composition study and execute the pre-committed FIRST-line
  gates BEFORE any expensive real-text run: (a) build the real and synthetic datasets every downstream claim depends on; (b)
  run the zero-LLM-spend a-priori envelope gate (T0) that decides whether a real-text headline exists and on which corpus
  (expected MATRES N*~0 vs NarrativeTime/TDDMan N*>>0); (c) de-risk the two non-real-text confirmatory claims — iteration
  gap (H3) and redundancy inverted-U (H4) — on clean synthetic ground truth with a parameterized simulated reader; and (d)
  map the recall-bite frontier to fix the LLM elicitation operating point. Together these pre-register and gate the entire
  study so iteration 2 can run the gated real-text comparison on the right corpus at the right operating point.
rationale: >-
  The hypothesis's own pre-committed tiering puts T0 FIRST as the gate that decides if a real-text headline is even possible:
  MATRES annotates only same/adjacent-sentence pairs, so its deduction-required envelope is empty by construction (N*~0),
  while NarrativeTime/TDDMan should have N*>>0. T0 is zero-cost and decisive — running it first prevents wasting the $10 LLM
  budget on a corpus that cannot host the claim, and it produces the hosting decision (which corpus is the real-text co-primary)
  plus the NarrativeTime non-circularity split. Two of the four confirmatory claims (H3 iteration gap, H4 redundancy inverted-U)
  are corpus-independent and can be established immediately on clean synthetic ground truth with a controlled simulated reader
  (exact knobs on per-edge recall and cross-edge correlation rho) — de-risking them before any corpus risk. The frontier pilot
  answers the other existential question — can the LLM emit SOUND but sub-universal disjunctive sets at usable recall with
  non-trivial singleton-resolution yield? — and is itself a pre-registered go/no-go for the main run. The datasets are the
  canonical reusable building blocks iterations 2+ depend on (T2 real-text comparison, T2b hallucination reduction, T4 real-text
  iteration stratum, LLM-on-synthetic). This is depth-first: every artifact either gates a go/no-go or builds load-bearing
  infrastructure, and LLM spend this iteration is confined to one cheap pilot.
artifact_directions:
- id: research_iter1_dir1
  type: research
  objective: >-
    Resolve every implementation/resource decision needed to build the closure engine, parse the three temporal corpora, realize
    synthetic networks, and configure baselines — so the dataset/experiment executors have exact specifications, not guesses.
  approach: >-
    Use the aii-web-tools skill (search -> fetch -> fetch_grep) to nail down, with exact citations/URLs/repo paths: (1) Access
    + format of the corpora — NarrativeTime/TimeBankNT (Rogers et al., LREC-COLING 2024; arXiv:1908.11443; locate the GitHub
    release + TimeML conversion tools and how to recover event nodes, gold relation edges, and per-pair sentence distance),
    TDDiscourse/TDDMan (Naik et al., 2019; aclanthology W19-5929; relation set {before,after,simultaneous,includes,is_included},
    mutually exclusive, no VAGUE), MATRES (Ning, Wu & Roth 2018; before/after/equal/vague, same/adjacent-sentence). (2) The
    EXACT composition tables: Allen's 13-relation interval algebra, the convex point algebra (<=,=,>= on points + disjunctions;
    the NarrativeTime start-point restriction; which non-convex relations widen to VAGUE), and RCC-8 — find authoritative
    machine-readable tables (GQR/SparQ distributions; Allen 1983). (3) Path-consistency spec: Mackworth PC-2 iterated to a
    fixpoint, converse/identity seeding from the algebra, the NAIVE single-pass-intersection variant (no fixpoint, no converse
    seeding), and tractability facts (PC complete for convex point algebra + ORD-Horn; NP-complete for full Allen IA / RCC-8).
    (4) Renz-Nebel random consistent QCN generator A(n,d,l) parameters for INDEPENDENTLY controlled redundancy/density/hop-count/cyclomatic
    number. (5) Baseline specs with a matched abstention signal each: Path-of-Thoughts (arXiv:2412.17963 — confirm per-path
    INDEPENDENT reasoning + path-agreement abstain), self-consistency vote margin, LINC multi-formalization vote (arXiv:2310.15164),
    DSR-LM induced rules (arXiv:2305.03742), a TempRel COMMIT baseline via ILP (arXiv:2502.11114; Knez & Sun arXiv:2406.11486),
    and the METRE multi-label head (arXiv:2408.07353). (6) The cheapest capable OpenRouter model for short-doc disjunctive
    relation reading + a prompt/response caching strategy keeping total spend well under $10. (7) Operationalization of the
    EXTENDED realism-matching statistic (total-variation distance of per-edge error-type distributions + cross-edge rho match
    + redundancy/topology histogram match). Deliver a structured dossier with a decision table and any verified gotchas.
  depends_on: []
- id: dataset_iter1_dir2
  type: dataset
  objective: >-
    Produce the canonical, schema-validated, fold-split gold relation graphs for NarrativeTime/TimeBankNT, TDDMan, and MATRES
    that all real-text experiments consume (T0 this iteration; T2/T2b/T4 in later iterations).
  approach: >-
    Download each corpus from its authoritative source (use the dataset executor's web tools for format details). For every
    document extract: event/timex nodes; gold relation edges in the corpus's native vocabulary; and per-edge metadata = sentence
    index of each event, an intra-/adjacent-/long-distance flag (the structural deduction-required proxy: do the two events
    share a local/adjacent-sentence span?), and document id. Map each corpus's native relations onto canonical algebra base-relation
    SETS: NarrativeTime -> interval Allen relations PLUS derived start-point convex-point relations (record the start-point
    ordering and the VAGUE-widening of non-convex {<,>}); TDDMan -> coarse interval algebra over {before,after,simultaneous,includes,is_included};
    MATRES -> its 4 relations. Emit rows {input: document text, output: gold graph (nodes + edges + relation sets), metadata_fold,
    corpus, per-edge sentence-distance + locally-justifiable-proxy flags}. Validate against the JSON schema (aii-json), split
    full/mini/preview, keep <=300MB (these corpora are small — hundreds of docs). Report per-corpus descriptive counts: events,
    edges, multi-path triangles, and >=3-edge/cyclic structures (descriptive only; T0 computes the gated N*). This is the
    frozen reusable foundation; do NOT compute derived gated statistics here.
  depends_on: []
- id: dataset_iter1_dir3
  type: dataset
  objective: >-
    Generate the clean-ground-truth synthetic backbone — random consistent qualitative constraint networks with INDEPENDENTLY
    controlled redundancy, density, hop-count, and cyclomatic number — plus natural-language realizations, as the canonical
    reusable corpus for the redundancy/iteration claims and for future LLM-on-synthetic reading.
  approach: >-
    Implement a Renz-Nebel-style A(n,d,l) generator producing random CONSISTENT networks (generate a base scenario, then weaken
    to a network that guarantees the gold singleton is contained on the query edge), for three algebras: Allen interval (13),
    convex point algebra, and RCC-8. Sweep grid cells over: number of constraining paths to the query pair (redundancy, several
    levels), path length / hop-count, cyclomatic number, and node count; target >=500 networks per cell. Per network record:
    the gold relation on every edge, the held-out query edge, the set of constraining paths through distinct intermediates,
    contributing-edge counts, and cycle structure. Produce TWO views per network: (a) the ABSTRACT graph (for simulated-reader
    experiments) and (b) an NL REALIZATION — template-verbalize each edge's relation into a short sentence pair so an LLM
    reader can be run on synthetic text in later iterations. Pre-register (record in metadata, do NOT validate yet) the realism-matching
    thresholds (TV distance of per-edge error-type distributions, cross-edge rho match, redundancy/topology histogram match)
    to be validated next iteration against the real frontier-pilot error distributions. Emit schema-valid rows {input: NL
    realization (abstract graph in metadata), output: gold graph, metadata_fold, algebra, redundancy/hop/cycle/node-count
    cell labels}. Validate with aii-json, split full/mini/preview, keep <=300MB.
  depends_on: []
- id: experiment_iter1_dir4
  type: experiment
  objective: >-
    Execute the decisive zero-LLM-spend a-priori envelope gate (T0) that decides whether a real-text headline exists and on
    which corpus, and validate the reusable closure engine on clean ground truth. This is THE pre-registered first gate; it
    gates iteration 2's real-text spend.
  approach: >-
    ENGINE: Implement the QCN path-consistency engine per the dossier spec — exact composition tables (Allen-13, convex point
    algebra, RCC-8), converse/identity seeding from the algebra, and three instrumented variants: FULL iterated PC (Mackworth
    PC-2 to fixpoint), NAIVE single-pass intersection at the query node (no fixpoint, no converse seeding), and closure-OFF.
    Unit-test on hand-checked small networks and confirm PC completeness on convex-point cases (runs in milliseconds/network).
    T0 (parse the three corpora inline from authoritative sources, mirroring the canonical DATASET artifact so they cross-validate):
    For NarrativeTime, TDDMan, and MATRES compute, from GOLD graphs ALONE (zero LLM spend), N* = count of held-out edges that
    SIMULTANEOUSLY (i) satisfy the structural deduction-required proxy (two events share no local/adjacent-sentence span),
    (ii) are constrained by >=2 paths through distinct intermediates, (iii) retain BITE after non-convex->VAGUE widening,
    (iv) have cross-path intersection equal to the gold singleton. Also compute: the >=3-edge/cyclic structural prevalence
    (the real-text iteration envelope for H3); the candidate count destroyed by widening (bite loss); a paired-bootstrap POWER
    calc per corpus; and — for NarrativeTime — the LOCALLY-JUSTIFIABLE vs PURELY-TIMELINE-IMPLIED split (non-circularity audit,
    structural proxy only). Score each corpus's deduction-required-multi-path-with-bite fraction against the pre-registered
    applicability NUMBER (>=10% general mechanism / 5-10% useful module / <5% niche safety-net). Emit method_out.json (aii-json
    validated) with the per-corpus envelope table, the hosting decision (which corpus is the real-text co-primary, if any
    clears the number), and the expected MATRES N*~0 vs NarrativeTime/TDDMan N*>>0 reported as gate validation. State the
    go/no-go verdict explicitly.
  depends_on: []
- id: experiment_iter1_dir5
  type: experiment
  objective: >-
    De-risk and establish H3 (iterated closure beats naive intersection, gap grows with hop/cyclomatic) and H4 (redundancy
    inverted-U with recall-dependent peak) on clean synthetic ground truth — independent of corpus and LLM-elicitation risk
    — using a parameterized simulated reader so per-edge recall and cross-edge correlation rho are exact knobs.
  approach: >-
    Generate random consistent QCNs inline with INDEPENDENTLY controlled redundancy/hop/cyclomatic structure (same generator
    family as the synthetic DATASET, abstract graphs only — no NL needed). Implement the same three closure variants (FULL
    / NAIVE / OFF) per the dossier spec. Simulate the LLM reader as a controlled channel: for each gold edge emit a disjunctive
    base-relation set parameterized by per-edge recall r = P(gold in emitted set) at >=4 FIXED levels, a breadth distribution,
    and a cross-edge correlation knob rho (positively-correlated within-document reading errors). (H3) Compute the FULL-minus-NAIVE
    selective-accuracy gap stratified by hop-count and cyclomatic number; verify the gap is ~0 on length-2 multi-path queries
    and GROWS with hop/cycle; run a monotone-trend test. (H4) Decompose net Mode-A gain into the narrowing BENEFIT curve and
    the silent-narrowing COST curve vs redundancy at each fixed recall; MEASURE empirical joint soundness J(E) = realized
    fraction of E-edge constraining subnetworks in which ALL edges are sound (do NOT assume the independence product r^E);
    the cost term is 1 - J(E); locate the inverted-U peak; verify the peak moves OUTWARD with higher recall and shifts under
    a recall-floor gate; confirm positive rho makes J(E) decay slower than r^E so the peak sits further out than the independence
    model predicts. Pre-register estimation precision (>=4 recall levels, >=500 networks/cell, >=1-bin minimum-detectable
    peak shift, peak-bin CI above both neighbors). Also run the zero-FP audit (C3): realized fraction-still-contains-gold
    vs EMPIRICAL J(E), report slope ~1 and apply the attribution rule (offset that disappears under J(E) = independence-approximation
    failure, not a soundness failure). Emit method_out.json (aii-json validated) with gap-vs-hop curves, benefit/cost decomposition,
    located peaks, audit slopes, and a clear PASS/FAIL on H3 and H4 at the pre-registered precision. Zero LLM spend.
  depends_on: []
- id: experiment_iter1_dir6
  type: experiment
  objective: >-
    Map the recall-bite frontier and fix the pre-registered LLM elicitation operating point — answering whether the LLM can
    emit SOUND but sub-universal disjunctive relation sets at usable per-edge recall with non-trivial singleton-resolution
    yield — BEFORE committing to the main run. This is the LLM-viability go/no-go.
  approach: >-
    Sample a modest set of edges by parsing a slice of each real corpus inline (NarrativeTime/TDDMan/MATRES) plus synthetic
    NL realizations. Via OpenRouter (cheapest capable model per the dossier; short docs; aggressive prompt/response caching
    to keep total spend well under $10), sweep a prompt-elicited BREADTH KNOB across >=5 settings from 'name THE single relation'
    -> 'name the maximal SOUND set of base relations the text does not exclude' (with an explicit underdetermined/universal
    option). For the NarrativeTime point-algebra arm, restrict emitted vocabulary to convex point relations on start-points
    (widen non-convex {<,>} to VAGUE) and measure the bite lost. At each setting MEASURE: per-edge RECALL = P(gold in emitted
    set); breadth distribution; over-commitment (sound-but-too-narrow) vs under-specification (universal) rates; raw closure-collapse
    rate (run a minimal closure check on available multi-path triangles); strict-tightening AND singleton-resolution-to-correct
    yields; a LOCAL-ONLY-reader accuracy probe (predict each edge from ONLY its local co-occurrence span) to empirically define
    the deduction-required fraction; and the within-document cross-edge reading-error correlation rho. PLOT the recall-bite
    frontier (recall vs singleton-resolution yield vs collapse rate) as a primary deliverable and apply the pre-registered
    go/no-go: proceed to the main run only if a region clears a recall gate AND yields non-trivial singleton-resolution-to-correct.
    Emit method_out.json (aii-json validated) with the frontier table, the SELECTED operating point, measured rho, the empirical
    deduction-required fraction, and cumulative OpenRouter spend. This operating point and rho feed iteration 2's real-text
    comparison and the synthetic realism validation.
  depends_on: []
expected_outcome: >-
  After this iteration we will have: (1) a frozen, schema-validated REAL-corpus dataset (NarrativeTime/TimeBankNT, TDDMan,
  MATRES) as gold relation graphs with deduction-required/sentence-distance metadata, and a frozen SYNTHETIC QCN corpus (Allen/convex-point/RCC-8)
  with independently controlled redundancy/hop/cycle structure + NL realizations — the reusable data foundation for every
  later experiment; (2) the T0 envelope-gate VERDICT: per-corpus N*, >=3-edge/cyclic prevalence, widening bite-loss, paired-bootstrap
  power, the NarrativeTime locally-justifiable vs purely-timeline-implied split, and the pre-registered hosting decision (which
  real corpus, if any, clears the applicability number) — the decisive go/no-go for a real-text headline at zero LLM cost,
  plus a validated, reusable closure engine (FULL/NAIVE/OFF); (3) synthetic confirmation/de-risking of H3 (full-minus-naive
  gap grows with hop/cyclomatic, ~0 at length-2) and H4 (redundancy inverted-U with recall-dependent, rho-shifted peak, audited
  against empirical J(E)) at the pre-registered estimation precision; (4) the recall-bite frontier map, the FIXED LLM operating
  point, measured rho, and the empirical deduction-required fraction — the viability gate for the real-text run; and (5) a
  complete implementation dossier. Iteration 2 can then run the GATED real-text Mode-A comparison vs Path-of-Thoughts and
  self-consistency at matched coverage (T2/H1), the end-to-end hallucination-reduction vs raw LLM (T2b/H2), and the real-text
  iteration stratum (T4/H3 on real), on the chosen corpus at the chosen operating point — depending on these frozen datasets,
  the engine, and the gate verdicts.
summary: >-
  A foundations-and-gates first iteration: build the real + synthetic datasets, run the zero-cost a-priori envelope gate (T0,
  expected MATRES N*~0 vs NarrativeTime/TDDMan N*>>0) that decides if/where a real-text headline lives, validate the reusable
  closure engine, de-risk the iteration (H3) and redundancy-inverted-U (H4) claims on clean synthetic ground truth with a
  controlled simulated reader, and map the recall-bite frontier to fix the LLM operating point — pre-registering and gating
  the entire study before any expensive real-text comparison, with LLM spend confined to one cheap pilot.
</previous_strategies>
...
### [2] HUMAN-USER prompt · 2026-06-17 15:16:14 UTC
...
### [3] SYSTEM-USER prompt · 2026-06-17 15:21:50 UTC
...
DEPENDENCY ERRORS (depends_on can ONLY reference IDs from <existing_artifacts>):
  - Strategy 1: Artifact 'experiment_iter2_dir3' (experiment): dependency 'art_K7riobQ_Rmwz' has type 'experiment' which is not allowed (allowed: {'research', 'dataset'})
  - Strategy 1: Artifact 'experiment_iter2_dir3' (experiment): dependency 'art_glhgFsBUrcYo' has type 'experiment' which is not allowed (allowed: {'research', 'dataset'})
  - Strategy 1: Artifact 'experiment_iter2_dir4' (experiment): dependency 'art_glhgFsBUrcYo' has type 'experiment' which is not allowed (allowed: {'research', 'dataset'})
  - Strategy 1: Artifact 'experiment_iter2_dir4' (experiment): dependency 'art_K7riobQ_Rmwz' has type 'experiment' which is not allowed (allowed: {'research', 'dataset'})
  - Strategy 1: Artifact 'experiment_iter2_dir4' (experiment): dependency 'art_TV5eEjdDP-Xp' has type 'experiment' which is not allowed (allowed: {'research', 'dataset'})
  - Strategy 1: Artifact 'experiment_iter2_dir5' (experiment): dependency 'art_K7riobQ_Rmwz' has type 'experiment' which is not allowed (allowed: {'research', 'dataset'})
  - Strategy 1: Artifact 'experiment_iter2_dir5' (experiment): dependency 'art_glhgFsBUrcYo' has type 'experiment' which is not allowed (allowed: {'research', 'dataset'})
