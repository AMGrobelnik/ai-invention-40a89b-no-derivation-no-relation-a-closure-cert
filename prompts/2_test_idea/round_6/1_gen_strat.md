...
### [1] SYSTEM-USER prompt · 2026-06-18 00:15:05 UTC
...
id: gen_strat_1_idx1
title: >-
  Run the Decisive Spatial RCC-8 Cross-Path-Intersection Test (SpaRTUN) to Settle the Fork, Deliver a GENUINE Fuzzy-Unification
  + Certificate-Bounds-Hallucination Result, Demonstrate the Full ~3000-Char End-to-End Pipeline, and Consolidate the One-Thesis
  Framing
objective: >-
  Settle the single load-bearing open question the reviewer named (MAJOR #1) and the hypothesis's primary fork (CLAIM 3):
  does the signature cross-path-intersection error-correcting-code mechanism transfer to a real(istic)-text venue, or is it
  synthetic-channel-only everywhere we can read? We attack it head-on with the one experiment the building blocks are already
  gated for: on the spatial RCC-8 venue (SpaRTUN / SpaRP-PS1, descriptively gated at a 27.4% TIGHT-bite GENERAL band in art_f-ofxduZjwSM),
  read constituent disjunctive RCC-8 sets span-locally with a real LLM and test AT POWER whether cross-path INTERSECTION narrows
  the deduction-required, multi-path-with-bite, gold-singleton query strictly BEYOND best-single-path composition (adjusted-CI
  separation), using the SAME a-priori gate, matched-coverage protocol, bracketing-CI discipline, synthetic positive control,
  and read-informativeness (recall+breadth) diagnostic as the temporal Allen test that came back negative. RCC-8 is the right
  venue because (i) the algebra is rich enough to give the symbolic step headroom and host multi-path bite, and (ii) constituent
  relations (containment, connection) are often STATED locally and may read far more informatively than Allen interval endpoints.
  The pre-registered FORK is publishable either way: POSITIVE -> the first real-venue demonstration that multi-path redundancy
  acts as an error-correcting code over LLM relational reads, moving the paper decisively above the bar; NEGATIVE (RCC-8 reads
  also underdetermine) -> a SECOND decisive negative that elevates the read-informativeness PRECISION/RECALL IMPOSSIBILITY
  from 'on temporal Allen' to 'everywhere we can currently read,' and re-centers the entire paper on the certificate + impossibility
  as THE contribution. Simultaneously we (a) convert reviewer MAJOR #2 (Mode-P is circular/memorized) from a self-negation
  into a real contribution by delivering a GENUINE fuzzy-unification result -- the LLM resolving semantically ambiguous /
  paraphrased / OUT-OF-TABLE relations at CALIBRATED (<1.0) confidence with the abstain-on-collapse certificate BOUNDING the
  resulting hallucination -- the goal element ('LLM as a probabilistic reasoning engine for fuzzy unifications, semantic similarities,
  logical gaps') that is otherwise entirely absent; (b) partly close the umbrella-goal gap (reviewer MINOR #4) with the FIRST
  end-to-end operational run of the full pipeline (unstructured ~3000-character professional document -> Prolog predicates
  -> SWI-Prolog reasoning -> human-auditable trace-graph -> quantified hallucination reduction vs raw LLM); and (c) land the
  one-thesis consolidation (MAJOR #3) and the small-realized-bite reporting fix (MINOR #5) at ZERO LLM spend.
rationale: >-
  The reviewer's #1 MAJOR is unambiguous and is also the hypothesis's organizing fork: the cross-path-intersection coding
  mechanism is powered ONLY on a synthetic channel, its real-text transfer FAILED on temporal Allen for a now-understood reason
  (LLM Allen reads of interval endpoints are near-universe -> zero realized bite), and the live hope rests on the un-run spatial
  RCC-8 test. That test is now FULLY DE-RISKED and ready to execute under <$10: the spatial multi-path-redundancy corpus is
  built and gated (art_f-ofxduZjwSM: SpaRP-PS1/SpaRTUN at 27.4% TIGHT-bite = GENERAL band, the strongest host; SpartQA-Human
  4.5% as a semi-natural secondary; StepGame/ReSQ = 0 honest boundaries), the corpus already ships src/algebra.py with an
  engine-validated RCC-8 table, the projection cardinal calculus is verified as a product of two point algebras so the validated
  point engine generates the cardinal table for free (art_2Xp7DiYUxoNo, with machine-readable GQR rcc8.comp/point.comp URLs
  and the verified 64-cell table), and the impl dossier supplies the Mackworth PC-2 closure pseudocode, the best-single-path
  / naive contrast, and the matched-abstention baseline configs (art_aQ2Rf8rwqteI). The reusable RCC-8 engine build + spatial
  read/answer/PoT prompts and the exact a-priori-gate / matched-coverage / bracketing-CI / synthetic-positive-control DRIVER
  also already exist in the iter-3 RCC-8 arm and the iter-4 decisive temporal test workspaces, which the executor can read
  directly. So the keystone experiment is execution-and-analysis on validated infrastructure, not new build-out. Crucially,
  RCC-8 is the venue where the read-informativeness diagnostic could break in our favor: the temporal negative was localized
  to Allen interval endpoints underdetermining text; RCC-8 containment/connection relations are frequently STATED, so the
  per-edge read-breadth measurement (the CLAIM-2 precision/recall diagnostic) is the attributable hinge -- tight, sound RCC-8
  reads + intersection bite => real-venue positive; near-universe reads + zero bite => second decisive negative. Both outcomes
  retire the MAJOR and keep the two-part thesis (certificate + impossibility) intact. The fuzzy-unification experiment is
  the second-highest-value move: reviewer MAJOR #2 showed Mode-P's precision 1.00 is CIRCULAR (the LLM MEMORIZED the kinship
  calculus at confidence exactly 1.0; on Allen the identical method collapses to 0.30), so 'fuzzy unification' was never actually
  demonstrated. A GENUINE case -- OUT-OF-TABLE vague spatial prepositions ('near','by','around','between') that have NO single
  RCC-8 answer, and ambiguous/paraphrased kinship terms ('folks','in-laws','uncle'=side-ambiguous) -- forces the LLM into
  a genuinely CALIBRATED (<1.0) disjunction, and the certificate's job is to BOUND the hallucination when that fuzzy read
  is unsound. This directly touches the goal element the motivation invokes and converts a criticism into a contribution.
  The ~3000-char demo is the only artifact that engages the FULL umbrella pipeline at the goal's stated document length (every
  prior venue falls short: CLUTRR <=871, spatial 130-1338), instantiating the paper's two-part thesis on a realistic professional
  document. The evaluation lands the remaining reporting fixes at $0. All four artifacts depend only on existing (frozen,
  validated) research/dataset/experiment artifacts and obey the dependency-type rules, so they run concurrently, reuse cached
  cost-guarded clients and validated engines, and combine into one pool that either earns the novel-mechanism headline on
  a real venue or honestly re-centers the paper -- under a hard <$10 cap per executor.
artifact_directions:
- id: experiment_iter5_dir1
  ...
  objective: >-
    ...
    reads also underdetermine -> the read-informativeness precision/recall impossibility holds everywhere we can currently
    read; drop the cross-path coding thesis from the headline and re-center on certificate + impossibility). Either way produce
    the attributable read-informativeness diagnostic (per-edge RCC-8 read recall AND breadth/underdetermined-rate) so the
    verdict is causally localized to whether constituents read locally.
  approach: >-
    ...
    as an exploratory natural-text check. The corpus ships src/algebra.py with an engine-validated RCC-8 table (64 cells);
    seed converses/composition from it and from the verified GQR rcc8.comp table in the spatial dossier [ARTIFACT:art_2Xp7DiYUxoNo],
    and for an OPTIONAL secondary cardinal-direction arm generate the cardinal composition table for free as a product of
    two point algebras per that same dossier. Take the Mackworth PC-2 iterated-closure pseudocode, the naive single-pass contrast,
    and the matched-abstention baseline configs from the impl dossier [ARTIFACT:art_aQ2Rf8rwqteI]; draw the synthetic RCC-8
    positive-control networks from the synthetic backbone [ARTIFACT:art_ghVQmxVlmOJJ] (synthetic_qcn_rcc8). For the EXACT
    reusable RCC-8 engine build (build_rcc8_algebra with the 64-cell self-test) + spatial read/answer/PoT prompts, read the
    iter-3 RCC-8-arm workspace at /ai-inventor/aii_data/runs/run_IuSkWzF0As-P/3_invention_loop/iter_3/gen_art/gen_art_experiment_3
    directly; for the a-priori-gate -> reads -> matched-coverage driver with the R1 bracketing-CI fix + Holm adjustment +
    synthetic positive control, read the iter-4 decisive-temporal-test workspace at /ai-inventor/aii_data/runs/run_IuSkWzF0As-P/3_invention_loop/iter_4/gen_art/gen_art_experiment_1
    ...
    power/MDE; pre-register the >=100-query power target and go/no-go. STEP 2 -- REAL LLM DISJUNCTIVE RCC-8 READS: prompt
    google/gemini-3.1-flash-lite (temp 0) AND a stronger cross-family reader (deepseek/deepseek-v3.2) to emit, per STATED
    relation between an object pair (span-local in the short story; identify mentions by surface form since SpaRP char_spans
    may be empty), a high-recall SOUND RCC-8 disjunction enumerating the 8 base relations + an explicit universal/underdetermined
    option ('name every RCC-8 relation the text does not rule out'); SHA-256 disk cache + hard cost guard; track cumulative
    spend, cap well under $10. CRITICAL DIAGNOSTIC (the CLAIM-2 hinge): measure per-edge RCC-8 read RECALL (P(gold in emitted
    set)) AND BREADTH / underdetermined-rate, for BOTH readers, exactly as the temporal Allen test did -- this is what attributes
    a win to locally-readable constituents vs a loss to near-universe reads. STEP 3 -- COMPARE at MATCHED single-relation
    coverage with doc-clustered paired bootstrap, Holm-adjusted: (a) CROSS-PATH INTERSECTION = full iterated PC intersecting
    all disjoint paths (Mode A, our mechanism); (b) BEST-SINGLE-PATH composition = the most-constraining single path's composed
    set (THE critical baseline isolating 'intersection beyond the single best path'); (c) NAIVE single-pass query-node intersection;
    (d) Path-of-Thoughts (per-path independent, modal vote, path-agreement abstain); (e) raw local LLM (forced single). HEADLINE
    CONTRAST = (a) vs (b): singleton-resolution-to-correct gap with a bracketing adjusted CI (percentile of the matched-gap
    distribution at a FIXED operating point; flag any CI excluding its own point). MINOR #5 DISCIPLINE: report BOTH the COVERAGE
    gain AND the SELECTIVE-ACCURACY gain for the synthetic RCC-8 positive control (intersection vs best-single-path) and for
    the real arm, so the modest realized bite is surfaced, not hidden. HONESTY: RCC-8 PC is sound-but-INCOMPLETE so coverage/collapse
    numbers are SOUND LOWER BOUNDS, but intersection-of-sound-sets is always sound so the narrowing is valid; tag every number
    REAL-LLM-READ (synthetic control REAL-LLM-READ-ON-SYNTHETIC); caveat clearly that SpaRP-PS1 is TEMPLATED/realistic not
    fully natural and docs are 130-1338 chars (a positive is 'realistic-text', NOT 'natural-3000-char-document'). Emit method_out.json
    (aii-json validated, exp_gen_sol_out) with per-query predict_intersection/best_single_path/naive/pot/raw + gold + path-structure
    metadata (n_disjoint_paths, per-path composed set, bite, singleton_resolved), the a-priori gate table, per-edge read recall+breadth
    (both readers, the precision/recall diagnostic), the intersection-vs-best-single-path leaderboard with bracketing adjusted
    CIs and BOTH coverage+selacc gains, the synthetic RCC-8 positive-control result, an auditable Prolog trace of one Mode-A
    narrowing + one Mode-B collapse, and an explicit CONFIRM (real-venue positive) / SCOPE-BOUNDARY (second decisive negative)
    FORK verdict with the read-informativeness attribution.
  depends_on:
  - id: art_f-ofxduZjwSM
    label: dataset
    relation_type:
...
- id: experiment_iter5_dir2
  ...
  approach: >-
    ...
    to', 'around', 'between', 'close to') that have NO single RCC-8 answer, so the LLM (google/gemini-3.1-flash-lite, temp
    0; cross-family deepseek-v3.2 sensitivity) MUST emit a CALIBRATED disjunction (e.g. 'near' -> {DC, EC} or {DC, EC, PO});
    gold = the actual stated RCC-8 relation. SETTING 2 -- AMBIGUOUS / PARAPHRASED KINSHIP: over CLUTRR chains [ARTIFACT:art_HS7-lxhZnU9m]
    ...
    and SHOW the confidence is genuinely sub-1.0 and reasonably calibrated -- the explicit contrast with memorized Mode-P
    (confidence exactly 1.0). (ii) feed the fuzzy disjunction + the other (exact-table) constituent reads into closure; the
    CERTIFICATE bounds hallucination: when the fuzzy read is UNSOUND (drops gold) closure either COLLAPSES (Mode-B detects,
    gold-free) or the query stays a disjunction (abstain), so confident-wrong is bounded; MEASURE recovered coverage, added
    confident-wrong, certificate-caught fraction, vs an abstain-always baseline and a commit-the-argmax baseline as a RISK-COVERAGE
    tradeoff (so answering more cannot trivially win). (iii) emit auditable trace-graphs that FLAG each LLM-resolved fuzzy
    step (with its <1.0 confidence) distinctly from exact-table steps. For reusable kinship/closure/calibration/LLM code,
    read the iter-4 fuzzy-unification workspace at /ai-inventor/aii_data/runs/run_IuSkWzF0As-P/3_invention_loop/iter_4/gen_art/gen_art_experiment_2
    ...
    exp_gen_sol_out) with per-instance fuzzy-read disjunction + confidence + gold, calibration curves (ECE, reliability),
    the certificate-bound risk-coverage table, the certificate-caught-fraction, flagged-fuzzy-step traces, and a verdict on
    whether genuine LLM fuzzy-unification adds net-faithful coverage with certificate-bounded hallucination.
  depends_on:
  - id: art_HS7-lxhZnU9m
    label: dataset
    relation_type:
...
- id: experiment_iter5_dir3
  ...
  objective: >-
    ...
    / collapse-certificate -> produce a human-auditable trace-graph -> report a QUANTIFIED hallucination reduction vs a raw
    LLM. Frame it as an operational case study that instantiates the paper's two-part thesis on realistic-length text (the
    certificate provides faithfulness-by-abstention; reads underdetermine on rich algebras), NOT a powered comparison.
  approach: >-
    ...
    gold; run the deduction module in the PC-COMPLETE convex POINT (start-point) algebra arm (tighter, more readable than
    full Allen -> non-trivial deduction yield, avoiding the Allen near-universe trap). OPTIONAL positive-contrast document
    = a ~3000-char kinship narrative (a long CLUTRR chain or a few concatenated CLUTRR sub-stories with consistent entity
    grounding) from [ARTIFACT:art_HS7-lxhZnU9m] where the calculus is memorized and the pipeline works fully, demonstrating
    the operational ceiling is extraction-limited not closure-limited. PIPELINE: follow the LOCAL-reader protocol, the pyswip-v0.3.3
    + subprocess SWI-Prolog discharge recipe (edge(E1,E2,RelSet) + algebra-seeded compose/3,converse/2; |R|==1 emit / |R|>1
    abstain / |R|==0 unsound-flag), and the CONFIDENT-WRONG matched-coverage definition from the iter-2 dossier [ARTIFACT:art_Dm5vYXmD1R8h],
    with the QCN engine/composition tables and baseline configs from the impl dossier [ARTIFACT:art_aQ2Rf8rwqteI]. For the
    exact reusable text->atoms->closure->Prolog-trace code, read the iter-3 CLUTRR end-to-end workspace at /ai-inventor/aii_data/runs/run_IuSkWzF0As-P/3_invention_loop/iter_3/gen_art/gen_art_experiment_1
    ...
    temp 0; SHA-256 cache, hard cost guard); report atomic-extraction P/R on the document (held-fixed control). (2) build
    the QCN, run PC-2 closure, emit per held-out query a singleton (emit) / non-singleton (abstain) / empty (Mode-B unsound-read
    ...
    on a handful of documents, NOT a powered statistical comparison; at ~0.53 atomic recall the deduction module abstains
    on much of the document and the certificate's value is faithfulness-by-abstention; state plainly which queries are extraction-limited
...
- id: evaluation_iter5_dir4
  type: evaluation
  objective: >-
    At ZERO LLM spend, land the reviewer's remaining reporting fixes and the one-thesis consolidation for GEN_PAPER_TEXT:
    (MINOR #5) surface the SMALL realized cross-path BITE of the synthetic positive control and across the inverted-U, tempering
    the coding-rate framing to 'precision of committed answers, not expanded coverage'; (MAJOR #3 support) produce the single
    ONE-THESIS contribution table with evidence tags moved into TABLE COLUMNS (not inline hedging), spine = the training-free/gold-free/per-edge
  ...
  approach: >-
    ...
    that even where the mechanism works the practical value is improved PRECISION of committed answers, not substantially
    expanded coverage -- pre-empting a skeptic who computes the modest realized bite. (MAJOR #3, one-thesis consolidation)
    Produce ONE contribution table whose COLUMNS are {claim, evidence-class tag (THEOREM / SYNTHETIC-CHANNEL / GOLD-ONLY-GATE
    / REAL-LLM-READ / REAL-LLM-READ-ON-SYNTHETIC), where-it-holds, status}, with the SPINE rows = (1) the certificate (CLUTRR
    confirmed at power, from [ARTIFACT:art_0a7i481ZRwS1]: vs-PoT +0.429 [0.299,0.563] Holm p_adj 0.0015; H2 absent-relation
    reduction +0.444 [0.317,0.583]; SWI-Prolog 40/40 engine, 39/40 gold; gold-read oracle 1.00@0.951) and (2) the read-informativeness
    precision/recall impossibility (temporal Allen near-universe, intersection 0/125, from [ARTIFACT:art_0AIWMhwc1pJM]; the
    spatial RCC-8 fork is the deciding pending row to be slotted by GEN_PAPER_TEXT), and SUPPORTING rows = inherited table-vs-LLM-composition
    scaling (point +0.043 -> RCC-8 +0.448 -> Allen +0.676) and the synthetic inverted-U. Re-affirm the corrected temporal
    statistics from the powered temporal experiment [ARTIFACT:art_OETjJkketEVS] (fixed-operating-point CI [-0.088,0.140] includes
    0, neither H1 gateway clears Holm; 42.5% confident-wrong-among-answered) and the deduction-sub-module + ~0.53-recall ->
    ~19%-coverage ceiling, all as table columns/footnotes rather than inline-everywhere. Emit eval_out.json (aii-json validated,
    exp_eval_sol_out) + eval_digest.md with the small-bite block (coverage-gain vs selacc-gain with CIs), the tags-in-columns
    one-thesis contribution table (with the spatial-RCC-8 fork row left as a labeled slot), the tempered inverted-U language,
    and explicit headline-structure guidance for GEN_PAPER_TEXT.
  depends_on:
  - id: art_0AIWMhwc1pJM
    label: synthetic-control
    relation_type:
  ...
  - id: art_OETjJkketEVS
    ...
    relation_rationale:
expected_outcome: >-
  After this iteration the paper's signature mechanism is either EARNED on a real venue or honestly closed, and every outstanding
  reviewer critique is retired. (1) THE FORK IS SETTLED: a powered, matched-coverage spatial RCC-8 experiment on SpaRTUN/SpaRP-PS1
  that for the first time compares cross-path INTERSECTION vs BEST-SINGLE-PATH on real(istic) spatial text with the read-informativeness
  (recall+breadth) diagnostic -- yielding either a CONFIRM (multi-path redundancy IS an error-correcting code over LLM relational
  reads; the synthetic-only central novelty becomes a real-venue positive, attributable to locally-readable RCC-8 constituents,
  with bracketing adjusted CIs and both coverage+selacc gains reported) or a defensible SECOND DECISIVE NEGATIVE (RCC-8 reads
  also underdetermine -> the precision/recall impossibility holds everywhere we can currently read; cross-path coding dropped
  from the headline, certificate + impossibility become THE thesis). (2) A GENUINE fuzzy-unification result: the LLM resolving
  out-of-table/vague spatial and ambiguous/paraphrased kinship relations at CALIBRATED (<1.0, ECE-measured) confidence with
  the abstain-on-collapse certificate BOUNDING the hallucination -- the goal element previously absent and the honest replacement
  for the circular Mode-P, with the memorized-calculus reframe stated plainly. (3) The FIRST end-to-end operational run of
  the complete pipeline (text -> Prolog predicates -> SWI-Prolog reasoning -> auditable trace-graph -> quantified hallucination
  reduction) on real ~3000-character professional documents, partially closing the umbrella-goal gap. (4) A $0 evaluation
  landing the small-realized-bite caveat (coverage gain ~2.4% vs the large selective-accuracy gain, with CIs and a tempered
  inverted-U), and the consolidated ONE-THESIS contribution table with evidence tags moved into table columns -- handing GEN_PAPER_TEXT
  a clean, single-spine narrative (certificate + read-informativeness impossibility) with the spatial fork slotted in. Net:
  the reviewer's MAJOR #1 fork is decided, MAJOR #2 is converted into a contribution, MAJOR #3 / MINORs #4-#5 are retired,
  and the paper moves decisively above the bar (positive fork) or becomes a tight, fully-honest certificate+impossibility
  paper (negative fork) -- all under a hard <$10 LLM budget per executor, reusing frozen engines, validated tables, and cached
  cost-guarded clients.
summary: >-
  Run the one experiment the reviewer demanded and the hypothesis pre-registered as the decisive fork: on the gated spatial
  RCC-8 venue (SpaRTUN/SpaRP-PS1, 27.4% tight-bite, engine+tables ready), read constituent RCC-8 disjunctions span-locally
  with a real LLM and test AT POWER whether cross-path INTERSECTION beats BEST-SINGLE-PATH (bracketing CIs, matched coverage,
  synthetic control, per-edge read recall+breadth diagnostic) -- CONFIRM the coding mechanism on a real venue or deliver a
  second decisive negative. In parallel: a GENUINE fuzzy-unification experiment (LLM resolves out-of-table/vague spatial +
  ambiguous kinship relations at calibrated <1.0 confidence with the certificate bounding hallucination) that converts reviewer
  MAJOR #2 into a contribution; the FIRST full end-to-end ~3000-char operational pipeline demo (text->Prolog->trace-graph->hallucination
  reduction) closing part of the goal gap; and a $0 evaluation landing the small-realized-bite caveat (MINOR #5) and the one-thesis
  tags-in-columns contribution table (MAJOR #3). Four focused artifacts; every dependency obeys the type rules (experiments
  depend only on datasets/research; the evaluation depends only on experiments), all on existing frozen artifacts, running
  in parallel under <$10.
</previous_strategies>
...
</prompt> so iter-3 can deliver — in ONE setting — all four numbers the umbrella names: (i) atomic-extraction precision/recall, (ii) multi-hop deduction accuracy vs chain length, (iii) a human-auditable trace-graph, and (iv) a hallucination-rate reduction on absent-relation queries.
...
DEPENDENCY ERRORS (depends_on can ONLY reference IDs from <existing_artifacts>):
  - Strategy 1: Artifact 'evaluation_iter6_dir3' (evaluation): dependency 'art_N8G6ZlQTONfk' has type 'evaluation' which is not allowed (allowed: {'dataset', 'experiment'})
  - Strategy 1: Artifact 'evaluation_iter6_dir3' (evaluation): dependency 'art_Vc1UBGIVSi0T' has type 'evaluation' which is not allowed (allowed: {'dataset', 'experiment'})
