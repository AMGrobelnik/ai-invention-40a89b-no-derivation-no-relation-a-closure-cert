...
type: evaluation
objective: >-
  At ZERO LLM spend, repair the RIGOR MAJOR and CLARITY MAJOR and the remaining minors, and hand GEN_PAPER_TEXT a clean, defensible
  scaffold (with a clearly-labeled DEMONSTRATED-FIX-PENDING slot the parallel extraction experiment fills). (1) [rigor MAJOR]
  Replace the located-in coverage-artifact headline: show that on a PURE-ABSENT pool confident-wrong equals coverage, so the
  '0.227 CW reduction at matched coverage / meets the 0.20 bar' sentence is vacuous (each signal's matched-operating-point
  CW = its natural CW = 0.30; modeA sits at coverage/CW 0.073), and ELEVATE the CAUGHT-FRACTION (certificate 0.785 vs verbalized
  0.400, verifier 0.274) as the genuinely matched-coverage-fair headline; reconcile with the artifact's own 'degenerate' note.
  (2) [clarity MAJOR] Regenerate a clean sec:boundary content block from intended content only and provide an end-to-end proofread
  checklist flagging any spliced/truncated text. (3) [clarity MINOR] Restate sibling-regime decisiveness via fabrication PREVALENCE
  (30% vs 6%) + the relatedness verifier being FOOLED by shared parent structure, acknowledging the abstention is still a
  structural no-path determination (just a non-trivial one needing the composition table, not mere disconnection). (4) [scope
  MINOR] Demote fuzzy unification to a one-line feasibility note. (5) [methodology MINOR] Scope the verifier claim precisely
  to 'a SAME-MODEL prompt-based verifier,' soften 'the established method' to 'a recognized prompt-based instantiation,' and
  add the stronger-cross-model-verifier PENDING slot. (6) [significance MAJOR fallback] Provide the abstract first-results-sentence
  framing FORK.
approach: >-
  ...
  and surface modeA at 0.073 coverage/CW. From the query-side verifier experiment [ARTIFACT:art_963U_7mCLAMJ] carry the per-venue
  caught-fraction leaderboards (cert 0.941/0.850 vs verifier 0.588/0.100) and the certificate-necessity verdict. From the
  iter-7 natural-kinship battery [ARTIFACT:art_htcr8yOZLCQy] recompute the Re-DocRED half of the per-signal x reader x corpus
  survival/caught table and the kinship mixed-pool reductions (-0.034..-0.055, CIs include 0), so the 16-cell capability-gap
  framing is rebuilt from source experiments. PRODUCE, all paper-facing: (i) the corrected located-in results table with CAUGHT-FRACTION
  as the matched-coverage headline and the '0.227 CW reduction' sentence DELETED (or relabeled 'certificate-at-its-abstaining-operating-point
  vs signal-at-natural-operating-point' with the pure-absent CW=coverage caveat); (ii) a clean regenerated sec:boundary block
  (net utility extraction-limited; Re-DocRED kinship reductions -0.034..-0.055 with CIs including 0 and Holm rejecting none;
  located-in present coverage 0.05 at extraction recall 0.148; gold-read ceiling 1.0/1.0/1.0 isolates extraction) + a full-draft
  end-to-end proofread checklist; (iii) the precise sibling-regime decisiveness paragraph; (iv) the one-line fuzzy feasibility
  note; (v) the precisely-scoped same-model-verifier language + the stronger-verifier PENDING slot; (vi) an updated one-thesis
  contribution table with a DEMONSTRATED-FIX-PENDING row the parallel extraction experiment fills; (vii) the abstract first-results-sentence
  FORK (demonstrated-fix wording IF the extraction experiment flips the mixed-pool reduction; otherwise the honest 'net utility
  demonstrated only on clean/templated graphs; natural-prose contribution = catching-objective win + quantified extraction
  boundary'). OUTPUT eval.py + eval_out.json (exp_eval_sol_out validated; full/mini/preview; every derived number under metadata
  with an evidence_tag) + eval_digest.md mirroring all blocks.
depends_on:
- id: art_l3mUIdHiAufv
...
objective: >-
  ...
  verifier, at MATCHED coverage with doc-clustered Holm-adjusted B=10000 CIs, EXCLUDES 0. Pre-register the FORK: a flip on
  EITHER domain converts 'diagnostic + boundary' into 'diagnostic + DEMONSTRATED FIX on natural prose' (the located-in sibling-regime
  flip is the most valuable); no flip yields a quantified recall-vs-reduction frontier showing exactly how much recall is
  needed. Secondarily run a STRONGER cross-model query-side false-premise verifier on the located-in sibling pool so the certificate's
  necessity claim is not scoped only to a same-model prompt verifier.
approach: >-
  ...
  absent; degenerate located_in/contains transitive table) and natural-kinship corpus [ARTIFACT:art_NUWTxBVWENIJ]. REUSE by
  direct filesystem read of the frozen iter-8 located-in harness at /ai-inventor/aii_data/runs/run_IuSkWzF0As-P/3_invention_loop/iter_8/gen_art/gen_art_experiment_1
  ...
  (d) multi-model UNION (primary google/gemini-3.1-flash-lite + a second family) with a PRECISION GUARD that keeps only edges
  above an agreement threshold / drops edges triggering spurious Mode-B collapse against high-agreement edges, so added recall
  does not inject false derivations. For each strategy x domain MEASURE: atomic P/R/F1 (converse-invariant primitive, + strict-direction
  + vs locally-justifiable subset), present coverage, certificate present selective accuracy, certificate absent confident-wrong,
  and the MIXED-POOL confident-wrong reduction + selective accuracy vs verbalized/sc_margin/P(True)/semantic-entropy AND vs
  the query-side verifier, all at MATCHED coverage with doc-clustered paired bootstrap B=10000 Holm-adjusted; carry the gold-read
  ceiling 1.0/1.0/1.0 as the upper bound. PRE-REGISTERED FORK (both publishable): IF any strategy on either domain makes the
  mixed-pool reduction CI EXCLUDE 0 -> verdict=DEMONSTRATED-FIX, reporting the recall threshold at which it flips and which
  regime (sibling-absent located-in is most valuable); ELSE -> verdict=EXTRACTION-LIMITED-BOUNDARY-CONFIRMED with the recall/reduction
  frontier quantifying the needed recall and the abstract calibrated accordingly. SECONDARY (methodology MINOR #5): run a
  STRONGER cross-model query-side verifier (a larger/different-family judge given the document + the pair: RELATEDNESS check
  + SELF-VERIFY pass, k=5 self-consistency) on the located-in sibling pool, grounded in the verifier spec [ARTIFACT:art_oUhZgMSjf7lm],
  and compare its caught-fraction to the certificate at matched coverage — report whether a stronger verifier closes the gap
  (honest negative) or not (strengthens certificate necessity). All reads REAL OpenRouter, SHA-256 disk cache, hard $9 cost-guard,
  gradual mini->full; total target ~$4-8 < $10. OUTPUT method_out.json (exp_gen_sol_out validated; full/mini/preview) with
  per-strategy per-domain atomic P/R, present coverage, mixed-pool leaderboards (Holm CIs), the recall-vs-reduction frontier
  table, the stronger-verifier caught-fraction comparison, carried gold-read ceiling, cross-reader block, cost ledger, and
  the explicit FORK verdict; include one worked present-recovery trace the improved extractor newly enables and one residual
  over-abstain trace, Prolog-discharged if swipl present else python-checked truthfully.
depends_on:
- id: art_RfjDpsGkBXDG
  label: dataset
  relation_type:
