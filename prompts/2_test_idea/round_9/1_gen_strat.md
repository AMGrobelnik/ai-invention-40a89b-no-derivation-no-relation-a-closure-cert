...
### [1] SYSTEM-USER prompt · 2026-06-18 06:16:45 UTC
...
summary: >-
  ...
  for verbalized/sc_margin/ptrue/negent — only P(True) partially separates them (median 0.0) yet 24.7% stilmonstrating the gate discriminates. SYNTHETIC PRIMARY (clean ground truth, controlled redundancy): a Renz-Nebel-style random consistent Allen/point QCN generator GUARANTEEING redundant paths/cycles and sweeping redundancy/density/hop-count INDEPENDENTLY; the NL-realization protocol is VALIDATED to clear the EXTENDED realism statistic; the redundancy DECOMPOSITION is reported at >=4 FIXED per-edge-recall levels with >=500 networks per cell. SECOND ALGEBRA: RCC-8 synthetic. ELICITED-TABLE VENUE: CLUTRR kinship (LLM-elicited table vs gold) doubling as an end-to-end hallucination-reduction-on-ABSENT-relation demo. TimeBank-Dense as a Tier-2 noisy-read-vs-clean-read arm; RuleTaker an out-of-scope propositional contrast.
...
id: gen_strat_1_idx1
title: >-
  Demonstrate a NET Certificate Win on a Non-Structural-by-Construction Natural Regime (Located-In Same-Component Siblings),
  Add the Query-Side False-Premise Verifier Baseline, and Re-Spine the Paper on the Mixed-Pool Capability Gap
objective: >-
  Convert the paper from 'corpus-robust diagnostic + candid negative' into 'diagnostic + a DEMONSTRATED FIX on a non-trivial
  natural regime' by executing the hypothesis's redefined CLAIM-2 decisive test (PATH 1) and simultaneously retiring all three
  reviewer MAJORs. Concretely: (1) [significance MAJOR] run the closure certificate vs the four-signal confidence battery
  vs a NEW query-side false-premise verifier on a MIXED present / SAME-COMPONENT-SIBLING-absent pool of the already-built
  natural located-in corpus, where absent-abstention is a GENUINE deductive result (co-component places, neither contains
  the other, the degenerate table leaves located_in∘contains UNDEFINED → sound EMPTY) rather than 'disconnected ⇒ trivially
  empty'; if the certificate's Holm-adjusted doc-clustered confident-wrong reductions EXCLUDE 0 there AND it matches/beats
  the query-side verifier, the headline converts to a demonstrated fix on a second natural domain (geographic/administrative
  containment), proving the diagnostic is not kinship-specific. (2) [novelty MAJOR] add the established method for exactly
  this failure mode — a query-side 'are these two entities related at all?' false-premise verifier + a self-verification pass
  — as a first-class baseline EVERYWHERE the certificate is compared (located-in now, and retro-fitted onto the cached CLUTRR
  + Re-DocRED kinship pools), and position the whole finding inside the false-premise / unanswerable-question abstention literature
  (FalseQA, AbstentionBench, query-side Wen2024). (3) [rigor MAJOR] drop 'the entire confidence family is structurally blind
  / reader-diverse' and re-spine the paper on the SIGNAL-AGNOSTIC mixed-pool capability gap (no single confidence threshold
  can both cover present and abstain on absent at matched coverage), reporting honest per-signal × reader × corpus crux-survival
  WITH fraction-CAUGHT, separating the definitional half from the empirical half. The novel knowledge becomes a compositional-false-premise
  DIAGNOSTIC plus a gold-free STRUCTURAL false-premise detector shown to net-beat both dispersion thresholds and a dedicated
  query-side verifier on a non-by-construction natural regime.
rationale: >-
  The reviewer verdict is precise and the fixes are now cheap and de-risked because every input exists and the engine is reusable
  verbatim. (i) Significance is the binding constraint: the certificate's only powered win (CLUTRR 0.827 vs 0.37-0.44) sits
  on the stratum conceded near-tautological (absent = disconnected components ⇒ sound closure abstains by definition), and
  on Re-DocRED kinship the mixed-pool reductions were -0.034..-0.055 (CIs include 0) because atomic recall 0.376 made the
  certificate over-abstain on present pairs. The hypothesis's PATH 1 is the clean fix and the corpus is ALREADY BUILT and
  gold-verified: art_RfjDpsGkBXDG ships 20,814 same_component_sibling pairs whose abstention is a GENUINE deductive result
  — I verified the degenerate containment_composition_table.json leaves located_in∘contains UNDEFINED so sibling/cousin places
  derive EMPTY through a real composition-refusal step, NOT through trivial disconnection (the dataset's own note states exactly
  this). located_in extraction is also expected to be EASIER than kinship (explicit cues 'X, a city in Y' / 'capital of',
  cue-present 100%, locally_justifiable_frac 0.588), so the positive fork is materially more reachable here than on kinship
  — and even if it returns extraction-limited again, the same_component_sibling result is the structurally honest place to
  prove the certificate's discipline survives when the graph is not handed to it. (ii) Novelty: an absent-relation query is
  a FALSE-PREMISE question, and the reviewer is right that the established detector for this is a query-side 'are X and Y
  related?' verifier, not just dispersion thresholds on a forced answer; omitting it leaves the comparison missing the obvious
  baseline and risks the DISCONFIRM that a cheap verifier already catches the fabrications. Adding it across all three venues
  (located-in via the new experiment, CLUTRR + Re-DocRED kinship via a cheap retrofit on the cached pools) makes the certificate's
  necessity testable, and a $0 literature artifact pins FalseQA/AbstentionBench/Wen2024 and carves the delta (compositional/multi-hop
  setting + gold-free training-free STRUCTURAL detector). (iii) Rigor: the paper's OWN cross-family data already refute family-level
  blindness — for deepseek the dispersion signals catch 67-90% of fabrications — so the defensible spine is the signal-agnostic
  mixed-pool capability gap, which the located-in experiment newly POWERS on a natural non-by-construction regime (it was
  previously powered only on clean CLUTRR). All harnesses are frozen and reusable: the four-signal battery + matched-coverage/crux-survival/mixed-pool/Holm
  stats from art_LeRQRGHJZcdQ (iter_6/gen_art/gen_art_experiment_1), the natural-corpus loader + name-grounding (dataio_redocred.py,
  grounding recall ~0.99) + certificate-abstention decomposition + gold-read ceiling + FORK harness from art_htcr8yOZLCQy
  (iter_7/gen_art/gen_art_experiment_1), and the kinship.py forward-union engine parameterized by the located-in table. Pacing:
  iters 8/9/10 remain; this front-loads the decisive measurement + the new baseline + the reframe scaffold NOW, leaving iter-9
  to unify the new located-in/query-side numbers into the re-spined paper and iter-10 for polish. Hard <$10 cap (realized
  ~$3-6). Experiments depend formally on datasets + the research methodology dossier; reused experiment harnesses are read
  directly from their frozen workspaces (experiment→experiment deps are not permitted).
artifact_directions:
- id: research_iter8_dir1
  type: research
  objective: >-
    Retire the NOVELTY MAJOR's literature half and design the new query-side baseline: (a) position confident absent-relation
    hallucination as a COMPOSITIONAL false-premise / unanswerable-question abstention failure and pin the closest-neighbor
    literature with verified BibTeX — FalseQA (Hu et al., 'Won't Get Fooled Again', ACL 2023), AbstentionBench (NeurIPS 2025),
    and the QUERY-SIDE (unanswerability / false-premise) content of Wen2024 ('Know Your Limits', TACL 2025) — with the exact
    claims, detection methods used, and quotable framing; (b) carve the genuine delta crisply: the compositional / multi-hop
    RELATIONAL setting (the false premise is specifically 'a derivation path exists between these entities', absent from sentence-level
    false-premise QA) and a GOLD-FREE, TRAINING-FREE STRUCTURAL false-premise detector (the no-derivation certificate) distinct
    from the trained/prompt-based detectors in that literature; (c) ground the design of the new query-side verifier baseline
    (a dedicated 'are these entities related at all?' relatedness check + a self-verification pass on the raw answer) in established
    self-verification / self-ask / unanswerability-detection methods so the experiments instantiate the recognized method,
    not a strawman.
  approach: >-
    Pure web research ($0, no code). Use the aii-web-tools skill (search → fetch → fetch_grep) and aii-semscholar-bib for
    BibTeX. WORKSTREAM 1 (false-premise / unanswerable literature): for FalseQA, AbstentionBench, and Wen2024, extract per
    paper: the precise framing of false-premise / unanswerable abstention, the detection method(s) studied (trained classifier
    vs prompt-based vs self-verification), headline findings (e.g. AbstentionBench's 'reasoning fine-tuning DEGRADES false-premise
    abstention'; FalseQA's finding that LLMs answer false-premise questions and detection is learnable; Wen2024's explicit
    query-side unanswerability/false-premise taxonomy), and 1-2 verbatim quotes each; verify venue/year/authors and emit project-key
    BibTeX. Explicitly correct the current paper's mischaracterization of Wen2024 as merely 'a confidence-threshold survey'
    — document its query-side content. WORKSTREAM 2 (delta carving): write a drop-in Related-Work paragraph connecting absent-relation
    hallucination to false-premise/unanswerable abstention and stating the two-part delta (compositional/multi-hop relational
    + gold-free structural detector), plus a one-sentence novelty claim that survives the reframe. WORKSTREAM 3 (query-side
    baseline design): survey query-side relatedness / self-verification / self-consistency-of-verification methods (e.g. self-verification,
    self-ask, P(IK)/P(True) applied to a relatedness question, two-step verify-then-answer) and recommend a concrete, cheap,
    prompt-based instantiation for BOTH the relation-relatedness verifier ('Based on the document, are X and Y related by
    <kinship|containment> at all? RELATED/UNRELATED + confidence') and the self-verification pass ('Is it actually true that
    X is <relation> to Y? Verify yes/no + confidence'), with a matched-coverage thresholding recipe, so the downstream experiments
    implement the established method faithfully. Build on the prior confidence-family dossier [ARTIFACT:art_dA_3iFe_7fn_].
    OUTPUT research_out.json {answer, sources, follow_up_questions} + research_report.md with the three workstreams, verified
    BibTeX block, the drop-in Related-Work paragraph, the delta statement, and the query-side baseline spec.
  depends_on:
  - id: art_dA_3iFe_7fn_
    label: extends
    relation_type:
...
- id: experiment_iter8_dir2
  type: experiment
  objective: >-
    THE decisive iter-8 result (significance MAJOR + novelty MAJOR + rigor MAJOR, on a SECOND natural domain): run the closure
    certificate vs the four-signal confidence/uncertainty battery vs a NEW query-side false-premise verifier on the genuinely-natural
    located-in (geographic/administrative containment) corpus, with the load-bearing comparison on a MIXED present / SAME-COMPONENT-SIBLING-absent
    pool where abstention is a GENUINE deductive result (not structural-by-construction). Deliver, exactly mirroring the CLUTRR/Re-DocRED
    battery sections: (i) FACT A on located-in (raw-LLM high-confidence located_in fabrication rate on same-component siblings)
    — domain-generality of the diagnostic; (ii) per-signal crux-survival WITH fraction-caught (rigor MAJOR + evidence MINOR);
    (iii) the signal-agnostic MIXED-POOL CAPABILITY GAP newly POWERED on a natural non-by-construction regime (certificate
    selective accuracy vs every signal at matched coverage); (iv) Holm-adjusted doc-clustered confident-wrong reductions of
    the certificate vs each signal AND vs the query-side verifier; and the pre-registered FORK that converts the headline.
  approach: >-
    INPUT = the built, gold-verified located-in corpus [ARTIFACT:art_RfjDpsGkBXDG] (re-docred PRIMARY slice; held_out + never_annotated
    present queries, same_component_sibling and different_component absent regimes; top-level metadata.composition_table =
    degenerate transitive containment table verbatim; engine reproduces present + derives EMPTY on absent). REUSE VERBATIM
    by direct filesystem read of frozen workspaces: the natural-corpus loader + entity-name GROUNDING (dataio_redocred.py,
    grounding recall ~0.99), the certificate-abstention DECOMPOSITION + gold-read ceiling + FORK orchestration + Prolog discharge
    from art_htcr8yOZLCQy at /ai-inventor/aii_data/runs/run_IuSkWzF0As-P/3_invention_loop/iter_7/gen_art/gen_art_experiment_1
    (method.py, readers.py, kinship.py forward-union engine, baselines.py, stats.py, llm.py, prolog.py); and the four-signal
    battery + crux-survival/mixed-pool/Holm statistics from art_LeRQRGHJZcdQ at /ai-inventor/aii_data/runs/run_IuSkWzF0As-P/3_invention_loop/iter_6/gen_art/gen_art_experiment_1
    (signals: verbalized confidence, self-consistency vote-margin@k=10, Kadavath P(True), semantic-entropy negentropy over
    k=10 relation-clustered samples; doc-clustered paired bootstrap B=10000, Holm). PARAMETERIZE kinship.py with containment_composition_table.json.
    PIPELINE: (1) span-local atomic located_in reads of each Wikipedia doc via OpenRouter — PRIMARY google/gemini-3.1-flash-lite
    (temp 0) AND cross-family deepseek/deepseek-v3.2 BOTH run for reader generality; SHA-256 disk cache + hard $9 cost-guard;
    gradual mini→full; for present held_out queries, DROP the single direct (source,target) atomic edge before querying (per
    the corpus spec) so the engine must deduce via an alternative ≥2-hop path. Add a BEST-EFFORT extraction arm (few-shot
    'enumerate every stated containment pair' prompt) alongside the default read to give the certificate its fair shot at
    the positive fork (PATH 2 hedge). Map atomic edges → forward-union closure recovers each present query / derives EMPTY
    on absent → certificate EMITs singleton / ABSTAINs on no-derivation / Mode-B on conflict. (2) RAW-LLM answerer + four
    confidence-thresholded raw-abstain baselines, tau swept to MATCH the certificate's coverage on present, on each absent
    regime, and on the mixed pool. (3) NEW QUERY-SIDE FALSE-PREMISE VERIFIER (the established method per the research dossier
    [ARTIFACT:art_dA_3iFe_7fn_]): a relatedness check ('Based on the document, is X located in / does it transitively belong
    to Y? RELATED/UNRELATED + confidence') and a self-verification pass on the raw committed answer, each thresholded to matched
    coverage — the certificate must MATCH or BEAT these. REPORT (every number tagged REAL-LLM-READ + NATURAL-PROSE + located-in
    domain): FACT A on same_component_sibling AND different_component (both readers, confidence distribution); per-signal
    x reader crux-survival table WITH fraction-CAUGHT (= 1 - survival) printed beside survival; the DECISIVE mixed present
    / same-component-sibling-absent showdown at matched coverage (certificate vs each of the 4 signals AND the query-side
    verifier) so abstain-on-everything cannot win; Holm-adjusted doc-clustered confident-wrong reductions with CIs; the SAME
    on a different_component-absent mixed pool as the structural-by-construction CONTRAST; natural-prose atomic located_in
    P/R/F1 (converse-invariant primitive); certificate abstention DECOMPOSITION (correct-absent vs over-abstain-present);
    present coverage + selective accuracy; and the GOLD-READ CEILING (closure over gold edges → 100% present / 100% absent
    abstention) to isolate extraction. PRE-REGISTERED FORK (both publishable): IF the certificate's Holm-adjusted confident-wrong
    reductions EXCLUDE 0 on the SAME-COMPONENT-SIBLING mixed pool AND it matches/beats the query-side verifier → emit verdict=DEMONSTRATED-FIX
    (the diagnostic + a net win on a non-trivial natural regime; new headline); IF extraction recall ties/loses → emit verdict=EXTRACTION-LIMITED-BOUNDARY
    while the DIAGNOSTIC (FACT A + the mixed-pool capability gap) still stands; ALSO flag if the query-side verifier already
    catches the fabrications as well as the certificate (→ honest negative on the certificate's necessity). OUTPUT method_out.json
    (aii exp_gen_sol_out, validated; full/mini/preview) grouping per-query rows into locatedin_present / locatedin_absent_sibling
    / locatedin_absent_diffcomponent with predict_certificate (+goldread), predict_conf_thresh_{verbalized,sc_margin,ptrue,negent},
    predict_queryside_verifier, predict_queryside_selfverify, predict_commit_argmax/raw, gold, per-signal confidences, metadata_regime
    / metadata_reader; plus the FACT-A table, fraction-caught crux table, mixed-pool leaderboards (sibling + diffcomponent)
    with Holm CIs, abstention decomposition, atomic P/R, gold-read ceiling, cross-reader comparison, cost ledger, and the
    explicit FORK verdict; include one worked same-component-sibling no-derivation abstention trace and one over-abstain-present
    trace, Prolog-discharged if swipl present else python-checked truthfully.
  depends_on:
  - id: art_RfjDpsGkBXDG
    label: dataset
    relation_type:
  ...
  - id: art_dA_3iFe_7fn_
    ...
    relation_type:
    relation_rationale:
- id: experiment_iter8_dir3
  type: experiment
  objective: >-
    Retire the NOVELTY MAJOR's baseline requirement on the two EXISTING powered venues and run the reviewer's DISCONFIRM test:
    does a dedicated query-side false-premise verifier already catch the confident absent-relation fabrications as well as
    the structural certificate? Add the query-side 'are these entities related at all?' verifier (+ self-verification pass)
    as a first-class baseline to the CACHED CLUTRR pool (180 absent + 102 present) and the Re-DocRED kinship pool (360 present
    + 368 absent), at matched coverage, so the certificate's claim is benchmarked against the established method for exactly
    this failure mode — not only dispersion thresholds on a forced answer — across all three venues when combined with the
    located-in experiment.
  approach: >-
    INPUT datasets = CLUTRR gold graphs [ARTIFACT:art_HS7-lxhZnU9m] and the natural kinship corpus [ARTIFACT:art_NUWTxBVWENIJ]
    (formal dataset deps). REUSE by direct filesystem read of frozen workspaces: the EXACT cached query pools + certificate/raw/signal
    predictions from the CLUTRR battery art_LeRQRGHJZcdQ (/ai-inventor/aii_data/runs/run_IuSkWzF0As-P/3_invention_loop/iter_6/gen_art/gen_art_experiment_1)
    and the Re-DocRED kinship battery art_htcr8yOZLCQy (/ai-inventor/aii_data/runs/run_IuSkWzF0As-P/3_invention_loop/iter_7/gen_art/gen_art_experiment_1),
    plus their llm.py (cached, cost-guarded OpenRouter client) and stats.py (doc/story-clustered paired bootstrap B=10000,
    Holm). A reproduction gate first re-loads each pool and asserts the carried certificate/raw/signal predictions and FACT-A/FACT-B
    literals match the published artifacts (0 mismatches, $0) before any new spend. NEW WORK (the ONLY new LLM calls, cheap):
    for every query pair in both pools, call the query-side false-premise VERIFIER ('Based on the story/document, are X and
    Y related by kinship at all? RELATED/UNRELATED + confidence') and a SELF-VERIFICATION pass on the raw LLM's committed
    answer ('Is it actually true that X is <relation> to Y? Verify + confidence'), per the design pinned in the research dossier
    [ARTIFACT:art_dA_3iFe_7fn_]; PRIMARY gemini-3.1-flash-lite + cross-family deepseek-v3.2; SHA-256 cache, hard $9 guard
    (estimated <$1 total since it is one extra call per query over ~282 + ~728 records, both readers). ANALYSIS: sweep the
    verifier's confidence threshold to MATCH the certificate's coverage on absent, present, and the mixed pool; report (a)
    the verifier's confident-wrong rate and selective accuracy vs the certificate and vs the four dispersion signals at matched
    coverage, with Holm-adjusted clustered-bootstrap CIs; (b) crux-survival of FACT-A fabrications under the verifier WITH
    fraction-caught; (c) a verdict per venue: does the query-side verifier MATCH/BEAT the certificate (→ structural certificate
    not strictly needed for that stratum, honest negative) or does the certificate still beat it (→ the structural signal
    is necessary)? This directly answers the reviewer's 'add at least one query-side baseline' and tests the explicit DISCONFIRM.
    OUTPUT method_out.json (exp_gen_sol_out, validated; full/mini/preview) grouping rows into clutrr_present / clutrr_absent
    / redocred_present / redocred_absent with predict_queryside_verifier, predict_queryside_selfverify (+ confidences) joined
    to the carried predict_certificate / predict_conf_thresh_* / gold, plus per-venue matched-coverage leaderboards (certificate
    vs 4 signals vs query-side verifier vs self-verify), fraction-caught crux tables, Holm CIs, the reproduction-gate report,
    cost ledger, and the per-venue certificate-necessity verdict.
  depends_on:
  - id: art_HS7-lxhZnU9m
    label: dataset
    relation_type:
  ...
  - id: art_dA_3iFe_7fn_
    ...
    relation_type:
    relation_rationale:
- id: evaluation_iter8_dir4
  type: evaluation
  objective: >-
    At ZERO LLM spend, execute the RIGOR-MAJOR reframe and the clarity/evidence/scope MINORs from EXISTING data and hand GEN_PAPER_TEXT
    a precise re-spining scaffold (with clearly-labeled PENDING slots the parallel located-in + query-side experiments fill
    in iter-9): drop 'the entire confidence family is structurally blind / reader-diverse'; pivot the spine to the signal-agnostic
    MIXED-POOL CAPABILITY GAP; build the honest per-signal × reader × corpus crux-survival table WITH fraction-CAUGHT; separate
    the definitional half of blindness from the empirical half and lead with the empirical; soften the 'removes none' overclaims;
    set the deduction-sub-module scope in the abstract's first sentence; and cut the concatenated-kinship operational arm.
  approach: >-
    Pure re-analysis (numpy+scipy, seed-fixed paired bootstrap, $0 LLM, no OpenRouter client instantiated; hard-assert spend==0);
    read prior artifact workspaces directly and reproduce-verify carried literals before extending. TASK 1 (rigor MAJOR —
    the spine pivot): from the CLUTRR battery [ARTIFACT:art_LeRQRGHJZcdQ] and the Re-DocRED kinship battery [ARTIFACT:art_htcr8yOZLCQy],
    build the explicit per-signal × reader × corpus crux-survival table printing BOTH survival AND fraction-caught (1 - survival):
    CLUTRR/gemini 0.435/0.718/0.247/0.718, CLUTRR/deepseek 0.672/0.224/0.103/0.224, Re-DocRED/gemini 0.508/0.850/0.483/0.850,
    Re-DocRED/deepseek 0.412/0.294/0.324/0.294 — and state honestly that (i) FACT A (the ~47%/32% high-confidence fabrication
    RATE) is the robust, corpus-AND-reader-transferable content, while (ii) FACT B (confidence cannot filter them) is READER-
    and SIGNAL-dependent: verbalized confidence is robustly blind, but the dispersion signals catch the MAJORITY (67-90%)
    for the stronger deepseek reader, so 'family-level blindness' is dropped. Re-write the headline as the SIGNAL-AGNOSTIC
    mixed-pool capability gap (no single confidence threshold can both cover present and abstain on absent at matched coverage),
    noting it is currently POWERED only on clean CLUTRR and that the parallel located-in experiment is landing it on a natural
    non-by-construction regime (PENDING slot). TASK 2 (clarity + evidence MINORs): separate the DEFINITIONAL half ('a high-confidence,
    self-consistent fabrication survives ANY dispersion threshold by construction' — state once, briefly, as the explanatory
    mechanism) from the EMPIRICAL half (how OFTEN fabrications are high-confidence and how strongly that varies by reader
    — lead with this); soften 'no signal removes a single one' to hold ONLY at the LLM's natural coverage, and report that
    at the CERTIFICATE's coverage P(True) already removes 75.3% on CLUTRR / 51.7% on natural prose; avoid the marginal '≥3
    of 4 keep ≥50%' framing. TASK 3 (scope MINOR): write the abstract FIRST-SENTENCE front-matter — closure-certified DEDUCTION
    SUB-MODULE, with extraction, OpenCyc/upper-ontology grounding, long-document operation, and general fuzzy unification
    explicitly OUT OF SCOPE, and (until the located-in fork lands) state the certificate's net utility is demonstrated only
    on clean/templated graphs with the natural result an extraction-gated boundary; recommend CUTTING the concatenated-kinship
    arm of the operational case study entirely (its 56/56 cross-story abstentions are trivial by construction) to reclaim
    space for the diagnostic and the second-domain replication. TASK 4 (novelty framing hooks): leave clearly-labeled PENDING
    slots and a one-thesis contribution table (evidence tags as COLUMNS) whose SPINE row is the compositional-false-premise
    DIAGNOSTIC + the gold-free STRUCTURAL detector, with the certificate mechanism conceded INHERITED (+0.673 inherited /
    +0.0025 novel), the query-side-verifier comparison and the located-in same-component-sibling net-win as named PENDING
    rows the iter-8 experiments fill, and the cross-path negative / mechanism analysis demoted to supporting/appendix rows.
    OUTPUT eval.py + eval_out.json (exp_eval_sol_out validated; full/mini/preview; every derived number under metadata with
    an evidence_tag) + eval_digest.md (paper-facing): the fraction-caught crux table, the capability-gap spine paragraph,
    the definitional-vs-empirical split, the softened-overclaim language, the abstract front-matter, the operational-arm cut,
    and the one-thesis table with PENDING slots.
  depends_on:
  - id: art_LeRQRGHJZcdQ
  ...
  - id: art_htcr8yOZLCQy
    ...
    relation_rationale:
expected_outcome: >-
  After this iteration the paper has (1) a NET certificate result on a non-structural-by-construction NATURAL regime: the
  located-in same-component-sibling experiment delivers FACT A on a second domain, the mixed-pool capability gap newly POWERED
  on natural prose, and Holm-adjusted doc-clustered certificate-vs-signal AND certificate-vs-query-side-verifier reductions
  with a pre-registered FORK — EITHER a DEMONSTRATED-FIX headline (reductions exclude 0 and the certificate matches/beats
  the verifier → 'diagnostic + demonstrated fix on a non-trivial natural domain') OR an honest extraction-limited boundary
  with the diagnostic intact, plus the different_component contrast quantifying exactly how much of the CLUTRR-style win was
  structural-by-construction; (2) the established query-side false-premise verifier baseline present in ALL THREE venues (located-in
  + retrofitted CLUTRR + Re-DocRED kinship), answering the novelty MAJOR's 'add the obvious baseline' and the DISCONFIRM 'does
  a cheap verifier already catch them?'; (3) a verified false-premise / unanswerable-question literature treatment (FalseQA,
  AbstentionBench, query-side Wen2024) with drop-in Related Work + carved delta; and (4) a $0 reframe scaffold that drops
  family-level blindness, re-spines on the mixed-pool capability gap, ships the per-signal × reader × corpus fraction-caught
  table, separates definitional from empirical, sets the deduction-sub-module abstract scope, and cuts the concatenated operational
  arm. Net: all three MAJORs (novelty / rigor / significance) and the clarity/evidence/scope MINORs are addressed with NEW
  measurement, not assertion, under a hard <$10 cap (realized ~$3-6) — leaving iter-9 to unify the located-in + query-side
  numbers into the re-spined paper and iter-10 for final polish.
summary: >-
  Execute the hypothesis's redefined decisive test (PATH 1): certificate vs the 4-signal battery vs a NEW query-side false-premise
  verifier on a mixed present / SAME-COMPONENT-SIBLING-absent pool of the already-built natural located-in corpus, where abstention
  is a genuine deductive result (verified: the degenerate containment table leaves located_in∘contains UNDEFINED → sound EMPTY),
  with a pre-registered FORK that converts the headline to 'diagnostic + demonstrated fix on a second natural domain' if the
  Holm-adjusted reductions exclude 0. In parallel: retrofit the query-side verifier onto the cached CLUTRR + Re-DocRED kinship
  pools (novelty-MAJOR baseline everywhere + the DISCONFIRM test), pin the false-premise/unanswerable literature ($0 research,
  carve the delta), and run a $0 reframe that drops 'family-level blindness', re-spines on the signal-agnostic mixed-pool
  capability gap, builds the per-signal×reader×corpus fraction-caught table, separates definitional from empirical, sets the
  deduction-sub-module abstract scope, and cuts the concatenated operational arm. Four focused parallel artifacts; experiments
  depend on existing datasets + the methodology dossier and read the frozen battery/engine/grounding harnesses directly; hard
  <$10.
</previous_strategies>
...
summary: >-
  ...
  TReMu (Ge2025, Findings-ACL 2025, 2025.findl survive. DECISIVE
...
### [2] HUMAN-USER prompt · 2026-06-18 06:16:45 UTC
...
### [3] SYSTEM-USER prompt · 2026-06-18 06:16:53 UTC

```
continue
```

### [4] SYSTEM-USER prompt · 2026-06-18 06:17:04 UTC

```
continue
```

### [5] SYSTEM-USER prompt · 2026-06-18 06:17:12 UTC

```
continue
```

### [6] SYSTEM-USER prompt · 2026-06-18 06:17:55 UTC

```
continue
```

### [7] SYSTEM-USER prompt · 2026-06-18 06:18:01 UTC

```
continue
```

### [8] SYSTEM-USER prompt · 2026-06-18 06:18:05 UTC
...
### [9] SYSTEM-USER prompt · 2026-06-18 06:20:12 UTC

```
continue
```

### [10] SYSTEM-USER prompt · 2026-06-18 06:25:23 UTC
...
DEPENDENCY ERRORS (depends_on can ONLY reference IDs from <existing_artifacts>):
  - Strategy 1: Artifact 'evaluation_iter9_dir3' (evaluation): dependency 'art_Kq_ROSdyIqPU' has type 'evaluation' which is not allowed (allowed: {'dataset', 'experiment'})
