...
### [1] SYSTEM-USER prompt · 2026-06-18 08:33:38 UTC
...
type: experiment
objective: >-
  THE significance lift (binding MAJOR): demonstrate a NET-UTILITY win OFF the structural-by-construction stratum by replacing
  prompt-only extraction with a PRECISION-PRESERVING SUPERVISED extractor — the gold-read-ceiling-identified binding lever
  (located-in recall 0.148, ceiling 1.0/1.0/1.0) — on the natural Re-DocRED located-in domain (PRIMARY, sibling regime) and,
  time permitting, kinship (SECONDARY), then re-run the MIXED-POOL (present + same-component-sibling-absent) confident-wrong
  reduction vs every dispersion signal AND vs the query-side verifier at MATCHED coverage with doc-clustered Holm-adjusted
  B=10000 CIs, sweeping the extractor's precision-preserving operating points to trace a recall→net-utility frontier. PRE-REGISTERED
  FORK: a flip (reduction CI excludes 0, and certificate matches/beats the verifier) on either domain => DEMONSTRATED-FIX-NATURAL-PROSE
  (headline converts to 'diagnostic + demonstrated fix', reporting the recall threshold R* where it flips); no flip => NET-UTILITY-BOUNDARY-STRUCTURAL
  (deeper than prompt-only — even precision-preserving recall cannot lift the reduction above 0), reported honestly while
  the diagnostic and the catching win still stand.
approach: >-
  INPUT = located-in corpus [ARTIFACT:art_RfjDpsGkBXDG] (gold docs + gold entity mentions + gold atomic located_in/contains
  edges + held_out/never_annotated PRESENT queries + same_component_sibling/different_component ABSENT pairs + SHA-256%5 folds
  + degenerate located_in/contains transitive table) and kinship corpus [ARTIFACT:art_NUWTxBVWENIJ] (SECONDARY domain). REUSE
  by direct filesystem read (NOT formal deps — read by absolute path): the iter-9 booster harness at /ai-inventor/aii_data/runs/run_IuSkWzF0As-P/3_invention_loop/iter_9/gen_art/gen_art_experiment_1
  (art_fXvxt4JO9hWy: booster.py per-strategy frontier + precision guard + mixed-pool matched-coverage confident-wrong reduction
  with doc-clustered Holm B=10000 CIs + gold-read ceiling + FACT-A / fraction-caught views, ALL reused verbatim), which imports
  the iter-8 located-in method.py at /ai-inventor/aii_data/runs/run_IuSkWzF0As-P/3_invention_loop/iter_8/gen_art/gen_art_experiment_1
  as core.py (kinship.py forward-union closure with the degenerate table, dataio_locatedin with held_out direct-edge ABLATION,
  readers_locatedin, queryside, baselines, stats, llm, prolog); and the FROZEN 1,215-query/283-doc competitor predictions
  from the iter-8 located-in run (art_l3mUIdHiAufv: raw commit, verbalized, sc_margin, P(True), semantic-entropy, query-side
  verifier, self-verify), which are REPLAYED at $0 from the SHA-256 cache and snapshot-asserted byte-identical (only the certificate/modeA
  predictions are recomputed). NEW CODE = extractor.py, a PRECISION-PRESERVING SUPERVISED extractor registered as one new
  strategy S_supervised alongside iter-9's S0-S5 prompt strategies. (1) TRAIN on docs DISJOINT from the iter-8 evaluation
  pool: read the 283 eval doc_ids from the per-query rows of the iter-8 located-in run and EXCLUDE them (and their fold) from
  training to prevent leakage. Task = document-level relation classification over GOLD-mention ordered entity pairs co-occurring
  within a context window: label located_in / contains / none, with adaptive per-relation thresholding for precision control.
  Two interchangeable, both precision-preserving (trained on the gold distribution) implementations — executor picks the more
  reliable: (a) a fine-tuned compact encoder (deberta-v3-small / roberta-base) with entity-marker inputs over local context
  (GPU via aii-parallel-computing; an off-the-shelf DocRED/Re-DocRED RE checkpoint as a fallback); (b) a calibrated gradient-boosted
  / logistic classifier over engineered entity-pair features (sentence co-occurrence + distance, located-in cue phrases, appositive/parenthetical
  patterns, NER subtype signatures, candidate-parent frequency) — CPU-cheap and robustly threshold-tunable. NO LLM, NO external
  KB, NO ontology. (2) EXTRACT all located_in/contains atomic edges per eval-pool doc over gold-mention pairs at SEVERAL precision-preserving
  operating points (sweep the decision threshold to trace a precision-recall curve from high-precision/low-recall up to higher-recall).
  (3) Feed the FULL extracted edge set per doc into the FROZEN certificate engine (kinship.py forward-union; for held_out
  present queries DROP the single direct (source,target) atomic edge before querying, per the corpus ablation contract); recompute
  ONLY the certificate predictions, competitors FIXED. MEASURE per operating point x domain: atomic P/R/F1 (converse-invariant
  primitive + strict-direction + vs locally-justifiable subset); certificate present coverage + present selective accuracy;
  certificate absent confident-wrong (sibling + diffcomponent); the MIXED-POOL confident-wrong REDUCTION + selective accuracy
  vs verbalized/sc_margin/P(True)/semantic-entropy AND vs the query-side verifier at MATCHED coverage (doc-clustered paired
  bootstrap B=10000, Holm-adjusted); the caught-fraction on sibling fabrications; and carry the gold-read ceiling 1.0/1.0/1.0
  as the upper bound. PLOT the PRECISION-PRESERVING recall→net-utility frontier and OVERLAY iter-9's PROMPT-ONLY frontier
  (negative slope -0.30/-0.67, precision-bought) so the contrast is explicit. Resolve the PRE-REGISTERED FORK from the data:
  DEMONSTRATED-FIX-NATURAL-PROSE if any precision-preserving operating point flips the mixed-pool reduction CI above 0 AND
  the certificate matches/beats the verifier (report R* and the certificate-minus-competitor reductions with Holm CIs); else
  NET-UTILITY-BOUNDARY-STRUCTURAL (the frontier quantifies the residual gap and localizes whether the limit is achievable
  recall or intrinsic present/absent confusion). Real reads only if unavoidable (target ~$0 via cache replay), SHA-256 disk
  cache, hard $9 cost-guard, gradual mini->full. OUTPUT method_out.json (exp_gen_sol_out validated; full/mini/preview) with
  per-operating-point per-domain atomic P/R, present coverage, certificate selective accuracy, mixed-pool leaderboards (Holm
  CIs), the precision-preserving recall→net-utility frontier table + the overlaid prompt-only contrast, the gold-read ceiling,
  the disjoint-fold/leakage-guard provenance, snapshot-equality assertions for the replayed competitors, the cost ledger,
  the FORK verdict, and worked traces (a present query the supervised extractor newly recovers -> certificate emits the correct
  singleton; a sibling no-derivation abstention the precise extractor preserves; Prolog-discharged if swipl present else python-checked,
  labelled truthfully).
depends_on:
- id: art_RfjDpsGkBXDG
  label: dataset
...
### [2] HUMAN-USER prompt · 2026-06-18 08:33:38 UTC
...
type: evaluation
objective: >-
  At ZERO LLM spend, retire the rigor MAJOR and all four MINORs and hand GEN_PAPER_TEXT a finished, defensible scaffold (with
  an abstract/contribution FORK keyed to the parallel precision-preserving-extractor experiment). (1) [rigor MAJOR] Report
  the stronger cross-family verifier on its OWN n=60 subsample in a SEPARATE table/footnote and correct tab:locatedin so 0.100
  is never placed beside the n=450/135 numbers (0.785/0.274). (2) [evidence MINOR] Pair the 0.785 caught win with the extraction-gated/targeting-quality
  caveat in abstract, intro, contribution-3, and conclusion. (3) [presentation MINOR] De-densify: tags into the spine table/headers
  only, delete inline markers and reviewer-response meta-narration, collapse the 7-paragraph intro. (4) [scope MINOR] Fold
  the operational ~3000-char note + the fuzzy note into one feasibility appendix; keep the out-of-scope statement in the intro.
  (5) [novelty MINOR slot] A labeled slot for the distinguishing sentence the parallel research artifact fills. (6) [significance]
  The abstract first-results FORK with an explicit selection rule keyed to the extractor experiment's verdict, plus the updated
  one-thesis contribution table.
approach: >-
  ...
  (1) [RIGOR MAJOR — load-bearing] From the iter-9 booster experiment [ARTIFACT:art_fXvxt4JO9hWy] stronger_verifier_block,
  recompute and emit a SEPARATE small table on the stronger-verifier's OWN n=60 subsample (n_sibling=250, 60 raw fabrications):
  certificate 0.667 / weak same-model verifier 0.20 / stronger cross-family deepseek-v3.2 k=5 0.10 / self-verify-strong 0.533,
  with an explicit header stating this denominator DIFFERS from the main n=450/135 sibling pool (cert 0.785 / verifier 0.274).
  Produce the CORRECTED tab:locatedin with the 'query-side verifier (stronger, x-family) 0.100' row REMOVED from the shared-denominator
  column and replaced by a clearly-labeled footnote pointing to the n=60 table; confirm the within-subsample qualitative conclusion
  (stronger verifier no better, both far below the certificate) holds. (2) [EVIDENCE MINOR] Provide the exact paired sentences
  for abstract / intro contribution-3 / conclusion that state 0.785 ALWAYS with the net-utility caveat (extraction-gated,
  NOT a deployment win) and the note that the catching objective on a pure-absent pool inherently favors high-abstention methods,
  so the win establishes TARGETING QUALITY not deployable net utility. (3) [PRESENTATION MINOR] A de-densification checklist
  + a rewritten lean intro skeleton: evidence-class tags ONLY in the spine table + section headers; delete inline \textsc{...}
  in results prose; rewrite meta-narration ('a skeptical reviewer rightly observed...', 'stated without spin', 'we never let
  that number carry the contribution') as declarative claims-with-caveats; collapse the 7 bold-headed intro paragraphs so
  contributions + headline numbers appear sooner. (4) [SCOPE MINOR] A feasibility-appendix consolidation plan folding the
  operational ~3000-char note (art_WQoePKrpsTPo) + the fuzzy-unification note (art_I22c-J7-OcXl / art_0MDLD-w-RXOu) into ONE
  short appendix, keeping the explicit out-of-scope statement (OpenCyc grounding, atomic re-extraction, general fuzzy unification,
  genuine ~3000-char documents) in the intro. (5) [NOVELTY MINOR slot] A clearly-labeled slot for the one distinguishing sentence
  (no-derivation certificate vs ontology-grounded post-extraction correction) that the parallel research artifact fills with
  verified content. (6) [SIGNIFICANCE — abstract FORK] Provide BOTH abstract first-results wordings with an explicit selection
  rule keyed to the precision-preserving-extractor experiment's verdict: IF DEMONSTRATED-FIX -> 'diagnostic + DEMONSTRATED
  net-utility fix on natural prose via a precision-preserving extractor (mixed-pool confident-wrong reduction CI excludes
  0 at recall R*, matching/beating the query-side verifier)'; ELSE -> 'diagnostic + targeting-quality catching win + a net-utility
  boundary shown FUNDAMENTAL even for precision-preserving extraction (deeper than prompt-only)'; update the one-thesis contribution
  table with the DEMONSTRATED-FIX-or-DEEPER-BOUNDARY row. Reproduce-verify against sources: caught-fractions and the pure-absent
  CW==coverage identity recomputed from the per-query rows of the iter-8 located-in run [ARTIFACT:art_l3mUIdHiAufv] (cross-checked
  against the iter-9 caught-fraction re-analysis art_4xy3D05YxvRr read by absolute path); the verifier-necessity leaderboards
  (cert 0.941/0.850 vs verifier 0.588/0.100) from [ARTIFACT:art_963U_7mCLAMJ]; the stronger-verifier block from [ARTIFACT:art_fXvxt4JO9hWy].
  OUTPUT eval.py + eval_out.json (exp_eval_sol_out validated; full/mini/preview; every derived number under metadata with
  an evidence_tag) + eval_digest.md mirroring all blocks paper-facing.
depends_on:
- id: art_fXvxt4JO9hWy
  label: rigor-source
  relation_type:
